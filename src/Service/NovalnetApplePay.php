...
class NovalnetApplePay extends AbstractNovalnetPaymentHandler implements SynchronousPaymentHandlerInterface
{
    ...
    {
        $sessionData = [];
        if ($this->helper->hasSession('novalnetapplepayFormData') && \version_compare($this->helper->getShopVersion(), '6.4.0', '<')) {
            $sessionData = $this->helper->getSession('novalnetapplepayFormData');
