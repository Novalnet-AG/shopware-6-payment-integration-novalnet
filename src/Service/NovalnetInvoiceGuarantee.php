...
class NovalnetInvoiceGuarantee extends AbstractNovalnetPaymentHandler implements AsynchronousPaymentHandlerInterface
{
    /** @var string */
    ...
    {
        $sessionData = [];
        if ($this->sessionInterface->has($this->paymentCode . 'FormData')) {
            $sessionData = $this->sessionInterface->get($this->paymentCode . 'FormData');
        }
        
        $formData = $this->sessionInterface->get($this->paymentCode . 'FormData');
		if (! empty($formData['dob'])) {
			$parameters['customer']['birth_date'] = date('Y-m-d', strtotime($formData['dob']));
		}
    }
