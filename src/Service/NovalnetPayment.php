    ...
    */
    ...
    public function transactionFailure(OrderTransactionEntity $orderTransaction, array $response, SalesChannelContext $salesChannelContext, $transaction = null, string $isExpress = null): ?CustomerCanceledAsyncPaymentException
    {
        ...
        foreach ([
			...
			if ($key == 'customerNo') {
				$insertData[$key] = !empty($response['customer'][$value]) ? $response['customer'][$value] : (!empty($requestParameter['customer'][$value]) ? $requestParameter['customer'][$value] : '');
			} else {
				$insertData[$key] = !empty($response['transaction'][$value]) ? $response['transaction'][$value] : (!empty($requestParameter['transaction'][$value]) ? $requestParameter['transaction'][$value] : '');
			}
		}
