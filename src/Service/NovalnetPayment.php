...
class NovalnetPayment implements AsynchronousPaymentHandlerInterface
{
    ...
    */
    ...
    public function handlePaymentProcess(SalesChannelContext $context, AsyncPaymentTransactionStruct $transaction, RequestDataBag $dataBag = null): array
    {
        ...
        $response = $this->helper->sendPostRequest($parameters, $this->helper->getActionEndpoint($paymentaction == 'authorized' ? 'authorize' : 'payment'), $paymentSettings['NovalnetPayment.settings.accessKey']);
    ...
    public function generateBasicParameters(SalesChannelContext $context, $transaction, array $paymentData, RequestDataBag $dataBag = null): array
    {
        ...
        if (!empty($dataBag->get('isRecurringOrder'))) {
            if (!in_array($paymentData['payment_details']['type'], ['INVOICE', 'PREPAYMENT'])) {
                $data = $this->transactionHelper->fetchNovalnetReferenceData($context->getCustomer()->getCustomerNumber(), $dataBag->get('parentOrderNumber'), $paymentData['payment_details']['type'], $context->getContext());
                ...
                if (!is_null($data)) {
                    ...
                    if (empty($parameters['customer']['billing']['company']) && !is_null($data->getAdditionalDetails())) {
		...
		if (!empty($paymentData['booking_details']['payment_action']) && $paymentData['booking_details']['payment_action'] === 'zero_amount' && $transaction->getOrder()->getPrice()->getTotalPrice() > 0) {
    ...
    */
    ...
    {  
        try {
            ...
            if (! empty($response['instalment']['cycles_executed'])) {
                $insertData['additionalDetails'] = $this->transactionHelper->getInstalmentInformation($response, $this->helper->getLocaleCodeFromContext($salesChannelContext->getContext(), true, $transaction != null ? $transaction->getOrder()->getLanguageId() : null));
            }
            ...
            if (!empty($transactionData)) {
                ...
                if(!isset($insertData['additionalDetails'])){
            ...
            $customFields = ['novalnet_comments' => $orderComments, 'novalnet_payment_name' => $this->helper->getUpdatedPaymentName($response['transaction']['payment_type'] ?? 'NOVALNET_PAYMENT', $locale), 'swag_paypal_resource_id' => (string) $response['transaction']['tid'], 'novalnet_payment_description' => $this->helper->getPaymentDescription($response['transaction']['payment_type'], $locale) ];
            ...
            if ((!empty($response['custom']['input2']) && $response['custom']['input2'] == 'subParentOrderNumber') || !empty($response['custom']['subParentOrderNumber'])) {
    ...
    public function transactionFailure(OrderTransactionEntity $orderTransaction, array $response, SalesChannelContext $salesChannelContext, AsyncPaymentTransactionStruct $transaction = null, string $isAdmin = null)
    {
        ...
        $customFields = ['novalnet_comments' => $orderComments, 'novalnet_payment_name' => $this->helper->getUpdatedPaymentName($response['transaction']['payment_type'] ?? 'NOVALNET_PAYMENT', $local), 'swag_paypal_resource_id' => (string) $response['transaction']['tid']];
