...
class NovalnetPayment implements AsynchronousPaymentHandlerInterface
{
    ...
    */
    ...
    public function transactionSuccess(OrderTransactionEntity $orderTransaction, array $response, SalesChannelContext $salesChannelContext, $transaction = null): void
    {
        try {
            ...
            if(!empty($response['event'])) {
				$getSalesChannel = $this->transactionHelper->getOrderCriteria($orderTransaction->getorderId(), $salesChannelContext->getContext());
				$paymentSettings = $this->helper->getNovalnetPaymentSettings($getSalesChannel->getSalesChannel()->getId());
			} else {
				$paymentSettings = $this->helper->getNovalnetPaymentSettings($salesChannelContext->getSalesChannel()->getId());
			}
            ...
            if ($response['transaction']['status'] === 'CONFIRMED' && !empty($response['transaction']['amount'])) {
                ...
                if (!empty($paymentSettings['NovalnetPayment.settings.completeStatus'])) {
                    ...
                    $paymentStatus = $this->transactionSuccessStatus($completeStatus);
                } else {
                    $paymentStatus = 'PAID';
                }
            }
            
            if ($response['transaction']['status'] === 'CONFIRMED' && $response['transaction']['amount'] == 0 && (in_array($response['transaction']['payment_type'], ['PREPAYMENT', 'INVOICE', 'PAYPAL']) || (in_array($response['transaction']['payment_type'], ['CREDITCARD', 'DIRECT_DEBIT_SEPA', 'GOOGLEPAY', 'APPLEPAY','DIRECT_DEBIT_ACH']) && (!empty($paymentdata['booking_details']['payment_action']) && $paymentdata['booking_details']['payment_action'] != 'zero_amount') && $orderTransaction->getAmount()->getTotalPrice() == 0))) {
                ...
                if (!empty($paymentSettings['NovalnetPayment.settings.onHoldStatus'])) {
                    $onHoldStatus = $paymentSettings['NovalnetPayment.settings.onHoldStatus'];
                    $paymentStatus = $this->transactionSuccessStatus($onHoldStatus);
                } else {
                ...
                $paymentStatus = 'AUTHORIZED';
            } elseif ($response['transaction']['status'] === 'CONFIRMED' && $orderTransaction->getAmount()->getTotalPrice() == 0){
            ...
            $paymentName = !empty($paymentdata['payment_details']['name']) ? $paymentdata['payment_details']['name'] : $this->helper->getUpdatedPaymentName($response['transaction']['payment_type']);
            
            $this->orderTransactionRepository->upsert([[
                'id' => $orderTransaction->getId(),
                'customFields' => [
                    'novalnet_comments' => $orderComments,
                    'novalnet_payment_name' => $paymentName,
                ]
    ...
    /**
     * Set transaction payment status
     *
     * @param string $status
     *
     * @return string
    ...
    public function transactionSuccessStatus(string $status) : ?string
    {
        $paymentStatus = '';
