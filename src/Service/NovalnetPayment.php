...
class NovalnetPayment implements AsynchronousPaymentHandlerInterface
{
    ...
    public function handlePaymentProcess(SalesChannelContext $context, AsyncPaymentTransactionStruct $transaction, RequestDataBag $dataBag = null): array
    {
        ...
        if (!empty($dataBag->get('novalnetpaymentFormData'))) {
            ...
            $parentOrderNo = $dataBag->get('parentOrderNumber');
            if (!empty($parentOrderNo)) {
                $subscription = $this->transactionHelper->getBookingInfoForRecurring($context->getContext(), $parentOrderNo, $transaction->getOrderTransaction()->getOrderId());
    ...
    public function generateBasicParameters(SalesChannelContext $context, $transaction, array $paymentData, RequestDataBag $dataBag = null): array
    {
        ...
        if (!empty($dataBag->get('isRecurringOrder'))) {
            ...
            if (!in_array($paymentMethod, ['INVOICE', 'PREPAYMENT'])) {
                $data = $this->transactionHelper->fetchNovalnetReferenceData($transaction->getOrder()->getOrderCustomer()->getCustomerNumber(), $dataBag->get('parentOrderNumber'), $paymentMethod, $context->getContext());
                if ($data !== null) {
     ...
     */
    public function checkTransactionStatus(OrderTransactionEntity $orderTransaction, array $response, SalesChannelContext $salesChannelContext, AsyncPaymentTransactionStruct $transaction, string $isAdmin = null): void
    {
     ...
     * @param AsyncPaymentTransactionStruct $transaction
     */
    public function transactionSuccess(OrderTransactionEntity $orderTransaction, array $response, SalesChannelContext $salesChannelContext, AsyncPaymentTransactionStruct $transaction): void
    {
        try {
            $paymentStatus = '';
            $paymentdata = [];
            ...
            if (!empty($response['instalment']['cycles_executed'])) {
                $insertData['additionalDetails']['InstalmentDetails'] = $this->transactionHelper->getInstalmentInformation($response, $this->helper->getLocaleCodeFromContext($salesChannelContext->getContext(), true, $transaction->getOrder()->getLanguageId()));
            }
            ...
            $orderComments = $this->helper->formBankDetails($response, $salesChannelContext->getContext(), $transaction->getOrder()->getLanguageId());
            ...
            if (in_array($response['transaction']['payment_type'], ['INVOICE', 'GUARANTEED_INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_INVOICE', 'INSTALMENT_DIRECT_DEBIT_SEPA', 'PREPAYMENT', 'MULTIBANCO']) && in_array($response['transaction']['status'], ['CONFIRMED', 'ON_HOLD', 'PENDING']) && !empty($transaction)) {
     ...
     * @param string|null $isAdmin
     */
    public function transactionFailure(OrderTransactionEntity $orderTransaction, array $response, SalesChannelContext $salesChannelContext, AsyncPaymentTransactionStruct $transaction, string $isAdmin = null)
    {
