...
abstract class AbstractNovalnetPaymentHandler
{
     ...
     * @return string
     */
    abstract public function prepareComments(array $response, Context $context): string;
     ...
     */
    public function checkTransactionStatus(OrderTransactionEntity $orderTransaction, array $response, SalesChannelContext $salesChannelContext, $transaction = null, string $isRecurring = null): void
    {
        if ($this->validator->isSuccessStatus($response)) {
            ...
            $this->transactionFailure($orderTransaction, $response, $salesChannelContext, $transaction, $isRecurring);
        }
    }
    ...
    public function transactionSuccess(OrderTransactionEntity $orderTransaction, array $response, SalesChannelContext $salesChannelContext, $transaction = null): void
    {
        try { 
            ...
            $orderComments = $this->prepareComments($response, $salesChannelContext->getContext());
     ...
     */
    public function transactionFailure(OrderTransactionEntity $orderTransaction, array $response, SalesChannelContext $salesChannelContext, $transaction = null, string $isRecurring = null): ?CustomerCanceledAsyncPaymentException
    {
        ...
        if(!is_null($transaction) && !empty($this->contextPersister) && $response['transaction']['payment_type'] !== 'APPLEPAY' && empty($isRecurring))
        {
    ...
    public function handleRedirectResponse(Request $request, SalesChannelContext $salesChannelContext, OrderTransactionEntity $orderTransaction): array
    {
        ...
        if (!empty($txnSecert) && $this->validator->isValidChecksum($request, $this->paymentSettings['NovalnetPayment.settings.accessKey'], $txnSecert)) {
