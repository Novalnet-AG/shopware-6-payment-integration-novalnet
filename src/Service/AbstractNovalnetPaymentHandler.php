...
abstract class AbstractNovalnetPaymentHandler
{
     ...
     * @param NovalnetPaymentTokenRepository $paymentTokenRepository
    ...
    public function __construct(
        ...
        NovalnetPaymentTokenRepository $paymentTokenRepository = null,
        ...
        if (!is_null($paymentTokenRepository)) {
     ...
     */
    abstract public function generatePaymentParameters(AsyncPaymentTransactionStruct $transaction, RequestDataBag $data = null, array &$parameters): void;
    ...
    public function handlePaymentProcess(SalesChannelContext $salesChannelContext, AsyncPaymentTransactionStruct $transaction, RequestDataBag $dataBag = null): array
    {
        ...
        $this->generatePaymentParameters($transaction, $dataBag, $parameters);
    ...
    public function generateBasicParameters(SalesChannelContext $salesChannelContext, AsyncPaymentTransactionStruct $transaction): array
    {
        ...
        $parameters['merchant'] = [
            'signature' => $this->paymentSettings['NovalnetPayment.settings.clientId'],
        ...
        if (!is_null($salesChannelContext->getSalesChannel()->getCurrency())) {
            $parameters['transaction']['currency'] = $salesChannelContext->getSalesChannel()->getCurrency()->getIsoCode();
        }
    ...
    public function setPaymentToken(string $paymentType, RequestDataBag $dataBag = null, array $sessionData, array &$parameters): void
    {
        ...
        } elseif ( (! empty($sessionData['saveData']) && $sessionData['saveData'] == 'on') || $parameters['transaction']['amount'] <= 0) {
    ...
    public function transactionSuccess(OrderTransactionEntity $orderTransaction, array $response, SalesChannelContext $salesChannelContext, AsyncPaymentTransactionStruct $transaction = null): void
    {
        try {
            ...
            if (! empty($response['customer']['customer_no'])) {
                $insertData['customerNo'] = $response['customer']['customer_no'];
            }
            ...
            $this->savePaymentToken($response, $this->sessionInterface->get('currentNovalnetPaymentmethod'), $salesChannelContext);
            ...
            if (!empty($paymentStatus)) {
				...
				if ( ($paymentStatus == 'PAYLATER' || ($paymentMethod == 'novalnetinvoiceguarantee' && $response['transaction']['status'] === 'CONFIRMED')) && !is_null($transaction))
				{
            ...
            throw new AsyncPaymentFinalizeException(
                ...
                sprintf('An error occurred during the communication with Novalnet%s%s', PHP_EOL, $e->getMessage())
            );
        }
    ...
    public function savePaymentToken(array $data, string $paymentMethod, SalesChannelContext $salesChannelContext): void
    {
        ...
        if (! empty($data['transaction']['payment_data']['token'])) {
            ...
            if (!empty($tokenData['accountData'])) {
                $this->paymentTokenRepository->savePaymentToken($salesChannelContext, $tokenData);
            }
        }
    }
    ...
    public function handleRedirectResponse(Request $request, SalesChannelContext $salesChannelContext): array
    {
        $this->paymentSettings = $this->helper->getNovalnetPaymentSettings($salesChannelContext->getSalesChannel()->getId());
        $response = [];
        if ($this->validator->isValidChecksum($request, $this->paymentSettings['NovalnetPayment.settings.accessKey'], $this->sessionInterface->get('novalnetTxnSecret'))) {
    ...
    public function getCustomerData(CustomerEntity $customerEntity)
    {
        ...
        if (!is_null($customerEntity->getActiveBillingAddress())) {
            ...
            if (!empty($customer ['billing']['company']) && !is_null($customerEntity->getActiveBillingAddress()->getVatId())) {
				$customer['vat_id'] = $customerEntity->getActiveBillingAddress()->getVatId();
            }
        }
