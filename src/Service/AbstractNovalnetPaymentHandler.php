...
abstract class AbstractNovalnetPaymentHandler
{
     ...
     */
    abstract public function generatePaymentParameters($transaction, array &$parameters, string $salesChannelId, ?RequestDataBag $data = null): void;
    ...
    public function handlePaymentProcess(SalesChannelContext $salesChannelContext, $transaction, ?RequestDataBag $dataBag = null): array
    {
        ...
        $this->generatePaymentParameters($transaction, $parameters, $salesChannelContext->getSalesChannel()->getId(), $dataBag);
     ...
     * @param array $parameters
     */
    public function setPaymentToken(string $paymentType, array $sessionData, array &$parameters, ?RequestDataBag $dataBag = null): void
    {
    ...
    public function transactionSuccess(OrderTransactionEntity $orderTransaction, array $response, SalesChannelContext $salesChannelContext, $transaction = null): void
    {
        try {
            ...
            if($response['transaction']['amount'] == 0 && !empty($transaction)){
                $response['isZeroAmountBooking'] = ($this->helper->amountInLowerCurrencyUnit($transaction->getOrder()->getPrice()->getTotalPrice()) > 0) ? true : false;
            }
