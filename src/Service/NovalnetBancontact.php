...
class NovalnetBancontact extends AbstractNovalnetPaymentHandler implements AsynchronousPaymentHandlerInterface
{
    
    /** @var string */
    protected $novalnetPaymentType = 'BANCONTACT';
    
    /** @var string */
    protected $paymentCode         = 'novalnetbancontact';

    /** @var string */
    protected $paymentHandler      = NovalnetBancontact::class;
    
    /** @var int */
    protected $position            = -1002;

    /** @var array */
    protected $translations        = [
        'de-DE' => [
            'name'        => 'Bancontact',
            'description' => 'Sie werden zu Bancontact weitergeleitet. Um eine erfolgreiche Zahlung zu gewährleisten, darf die Seite nicht geschlossen oder neu geladen werden, bis die Bezahlung abgeschlossen ist',
        ],
        'en-GB' => [
            'name'        => 'Bancontact',
            'description' => 'You will be redirected to Bancontact. Please don’t close or refresh the browser until the payment is completed',
        ],
    ];
    ...
    public function pay(AsyncPaymentTransactionStruct $transaction, RequestDataBag $dataBag, SalesChannelContext $salesChannelContext): RedirectResponse
    {
        ...
        {
			if (version_compare($this->helper->getShopVersion(), '6.4.2.0', '>=')) {
				$this->contextPersister->save($transaction->getOrderTransaction()->getId(), ['novalnetErrorMessage' => $response['result']['status_text']], $salesChannelContext->getSalesChannel()->getId());
			}
            $this->sessionInterface->set('novalnetErrorMessage', $response['result']['status_text']);
