...
class NovalnetPaypal extends AbstractNovalnetPaymentHandler implements AsynchronousPaymentHandlerInterface
{
    ...
    public function generatePaymentParameters(AsyncPaymentTransactionStruct $transaction, RequestDataBag $data = null, array &$parameters, array $paymentSettings): void
    {
        ...
        } elseif($data->get('novalnetpaypalFormData')) {
			$sessionData = $data->get('novalnetpaypalFormData')->all();
		} elseif (!empty($_REQUEST['novalnetpaypalFormData'])) {
			$sessionData = $_REQUEST['novalnetpaypalFormData'];
		}
