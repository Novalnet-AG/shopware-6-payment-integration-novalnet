...
class NovalnetFilter extends AbstractExtension
{
    ...
    {
        $sessionData = [];
        $paymentMethod= $this->helper->getPaymentMethodName(
        ...
        if (! empty($paymentMethod) && $this->validator->checkString($paymentMethod)) {
            ...
            if (!empty($sessionData) && ! empty($sessionData['accountData'])) {
                $sessionData['accountData'] = $this->helper->getLastCharacters(
                    $this->helper->formatString($sessionData['accountData'], ' ')
                );
                $data = [$sessionData];
     ...
     * @return array
     */
    public function getNovalnetInstalmentInfo(SalesChannelContext
                    $salesChannelContext, $orderNumber): array
    {
        ...
        if (!is_null($transactionData) && $transactionData->getGatewayStatus() === 'CONFIRMED') {
