...
class NovalnetValidator
{
    ...
    {
        if (!$this->helper->getSupports('guarantee', $paymentMethod)) {
            return 'NO';
        }
        ...
        if (empty($settings[$paymentMethod]['minimumOrderAmount'])) {
            $settings[$paymentMethod]['minimumOrderAmount'] = 999;
        }
        if($orderAmount <= 0){
		return 'NO';
	}	

        if (0 < (int) $settings[$paymentMethod]['minimumOrderAmount'] && (int) $settings[$paymentMethod]['minimumOrderAmount'] > (int) $orderAmount) {
            return 'NO';
        }
        
        if (!empty($settings[$paymentMethod]['allowB2B'])) {
     ...
     * @return bool
     */
    public function isValidEmail($mail, $paymentSettings): bool
    {
        return (bool) (new EmailValidator())->isValid($paymentSettings['NovalnetPayment.settings.mailTo'], new RFCValidation());
    }
}
