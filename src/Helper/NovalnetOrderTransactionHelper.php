...
class NovalnetOrderTransactionHelper
{
    ...
    public function sendMail(SalesChannelContext $salesChannelContext, OrderEntity $order, string $note , bool $instalmentRecurring = false): void
    {
		...
		$senderName = $salesChannelContext->getSalesChannel()->getName() ? $salesChannelContext->getSalesChannel()->getName() : $salesChannelContext->getSalesChannel()->getTranslated()['name'];
        $renderedTemplate = $this->twig->render('@NovalnetPayment/documents/mail.html.twig', ['note' => $note, 'order' => $order, 'salesChannel' => $salesChannelContext->getSalesChannel(), 'instalment' => $instalmentRecurring, 'context' => $salesChannelContext]);
        ...
        {
			$data->set('subject', sprintf($this->translator->trans('NovalnetPayment.text.instalmentMailSubject'), $senderName, $order->getOrderNumber()));
		} else {
    ...
    public function prepareMailContent(OrderEntity $order, SalesChannelContext $salesChannelContext, string $note, $instalmentRecurring = false): void
    {
        if (!is_null($order->getOrderCustomer())) {
            ...
            try {
                $emailConfigs = $this->helper->getNovalnetPaymentSettings($salesChannelContext->getContext()->getSource()->getsalesChannelId());
    ...
    public function refundTransaction(NovalnetPaymentTransactionEntity $transactionData, OrderTransactionEntity $transaction, Context $context, int $refundAmount = 0, Request $request = null, bool $instalmentCancel = false) : array
    {
        ...
        if ($this->validator->isSuccessStatus($response)) {
            if (! empty($response['transaction']['status'])) {
                ...
                {
					$message .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.instalmentRefundComment'), $transactionData->getTid(), date('Y-m-d H:i:s'), $refundedAmountInBiggerUnit);
				} else {
