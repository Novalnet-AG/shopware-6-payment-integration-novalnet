...
class NovalnetOrderTransactionHelper
{
    ...
    */
    ...
    public function fetchNovalnetTransactionData(string $orderNumber, $context = null, string $tid = null) : ? NovalnetPaymentTransactionEntity
    {
        ...
        $criteria->addFilter(new NotFilter('AND', [new ContainsFilter('novalnet_transaction_details.additionalDetails', 'change_payment')]));
     ...
     * @return string
     */
    public function getChangedPaymentName(SalesChannelContext $salesChannelContext, string $orderNumber): ?string
    {
        $paymentMethodName = '';
        $transactionData = $this->fetchNovalnetTransactionDataForTemplate((string) $orderNumber, $salesChannelContext->getContext());
        if (!empty($transactionData) && !empty($transactionData->getAdditionalDetails())) {
            $additionalDetails = $this->helper->unserializeData($transactionData->getAdditionalDetails());
            $paymentMethodName = !empty($additionalDetails['payment_name']) ? $additionalDetails['payment_name'] : '';
        }
                ...
                if (! empty($response['transaction']['status'])) {
                     ...
                     if ($transactionStatus === 'CONFIRMED') {
                         if (!empty($paymentSettings['NovalnetPayment.settings.completeStatus'])) {
                             ...
                             $this->managePaymentStatus($completeStatus, $paymentType, $transaction->getId(), $context);
                         } else {
                             $this->orderTransactionState->paid($transaction->getId(), $context);
                         }
                     } elseif ($transactionStatus === 'PENDING') {
                         $this->managePaymentStatus('process', $paymentType, $transaction->getId(), $context);
                     } elseif ($transactionStatus !== 'PENDING') {
                         $this->orderTransactionState->cancel($transaction->getId(), $context);
                     }
                }
    ...
    */
    ...
    public function bookOrderAmount(NovalnetPaymentTransactionEntity $transactionData, OrderEntity $order, Context $context, int $bookAmount) : array
    {
        ...
        if (!empty($transactionData->getAdditionalDetails())) {
            ...
            $parameters['transaction']['payment_data'] = ['token' => !empty($parameter['token']) ? $parameter['token'] : $transactionData->getTokenInfo()];
            unset(
            ...
            if ($this->helper->isSuccessStatus($response)) {
                ...
                try {
                    if (!empty($paymentSettings['NovalnetPayment.settings.completeStatus'])) {
                        $completeStatus = $paymentSettings['NovalnetPayment.settings.completeStatus'];
                        $this->managePaymentStatus($completeStatus, $transactionData->getPaymentType(), $transaction->getId(), $context);
                    } else {
                         $this->orderTransactionState->paid($transaction->getId(), $context);
                    }
                } catch (IllegalTransitionException $exception) {
    ...
    public function getSubscriptionDetails(Context $context, string $orderNumber) : array
    {
        ...
        if (!empty($subscription['subscription'])) {
            ...
            if (!empty($subscription['subscription']['booking_details'] ['due_date'])) {
                $datas['booking_details']['due_date'] = $subscription['subscription']['booking_details'] ['due_date'];
            }
        } else {
            $datas = [
                'payment_details' => [
     ...
     *
     */
    public function managePaymentStatus(string $status, string $paymentType, string $transactionId, Context $context)
    {
        ...
        if ($status == 'paid') {
            ...
            if (in_array($paymentType, ['INVOICE', 'PAYPAL', 'PREPAYMENT'])) {
                try {
                    $criteria = new criteria();
                ...
                $this->orderTransactionState->process($transactionId, $context);
            }
        }
    }
}
