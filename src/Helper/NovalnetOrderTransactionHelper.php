...
class NovalnetOrderTransactionHelper
{
     ...
     */
    public function sendMail(Context $context, OrderEntity $order, string $note, bool $instalmentRecurring = false): void
    {
        ...
        if (strpos($transaction->getPaymentMethod()->getHandlerIdentifier(), 'Instalment') !== false) {
            $instalmentInfo = $this->getNovalnetInstalmentInfo($context, $order->getOrderNumber());
        }

        $mailTemplate =  $this->getMailTemplate($context, 'novalnet_order_confirmation_mail');
        ...
        if (!empty($mailTemplate->getMedia()) && !empty($mailTemplate->getMedia()->first())) {
            $data->set('binAttachments', $this->helper->getMailAttachment($mailTemplate, $context));
        }
    ...
    public function getQrImage(Context $context, string $orderNumber): string
    {
        ...
        if (!empty($transactionData)) {
            if (!in_array($transactionData->getGatewayStatus(), ['DEACTIVATED', 'FAILURE']) && (in_array($transactionData->getPaymentType(), ['novalnetinvoice', 'novalnetprepayment']) || (in_array($transactionData->getPaymentType(), ['novalnetinvoiceguarantee', 'novalnetinvoiceinstalment']) && $transactionData->getGatewayStatus() !== 'PENDING'))) {
                $serilazedData = $this->helper->unserializeData($transactionData->getAdditionalDetails());
                if (!empty($serilazedData['qr_image'])) {
                    return $serilazedData['qr_image'];
                }
            }
        }
        return '';
     ...
     *
     */
    public function prepareMailContent(OrderEntity $order, Context $context, string $note, $instalmentRecurring = false): void
    {
        if (!is_null($order->getOrderCustomer())) {
            $orderReference = $this->helper->getOrderCriteria($context, $order->getId(), $order->getOrderCustomer()->getCustomerId());
            try {
                $emailMode = $this->helper->getNovalnetPaymentSettings('NovalnetPayment.settings.emailMode', $order->getSalesChannel()->getId());
                if (!empty($emailMode)) {
                    $this->sendMail($context, $orderReference, $note, $instalmentRecurring);
                }
            } catch (\RuntimeException $e) {
                //Ignore
            }
     ...
     */
    public function fetchNovalnetTransactionData(?string $orderNumber = null, ?Context $context = null, ?string $tid = null, bool $changePayment = false): ?NovalnetPaymentTransactionEntity
    {
        $criteria = new Criteria();
                ...
                if (! empty($response['transaction']['status'])) {
                    ...
                    if (in_array($response['transaction']['status'], ['CONFIRMED', 'PENDING'])) {
                        ...
                        if (!empty($transactionData->getAdditionalDetails()) && !empty($transactionData->getPaymentType()) && in_array($transactionData->getPaymentType(), ['novalnetinvoice', 'novalnetinvoiceguarantee', 'novalnetprepayment', 'novalnetinvoiceinstalment'])) {
                            ...
                            $upsertData['additionalDetails'] = $response['transaction']['bank_details'];
                        ...
                        $appendComments = false;
                        $message .= $this->helper->formBankDetails($response, $context);
                    ...
                    if ($response['transaction']['status'] === 'CONFIRMED') {
                        ...
                        $this->helper->setStatus($transaction->getId(), 'in_progress', $context);
                    } elseif ($response['transaction']['status'] !== 'PENDING') {
                        $this->orderTransactionState->cancel($transaction->getId(), $context);
                    }
                }
    ...
    public function bookOrderAmount(int $amount, NovalnetPaymentTransactionEntity $transactionData, OrderEntity $order, Context $context): array
    {
        ...
        // payment name using the payment code
        $paymentCode = $this->helper->formatString($transactionData->getPaymentType());

        // Built merchant parameters.
        $parameters['merchant'] = [
            'signature' => str_replace(' ', '', $this->helper->getNovalnetPaymentSettings('NovalnetPayment.settings.clientId', $order->getSalesChannelId())),
            'tariff'    => $this->helper->getNovalnetPaymentSettings('NovalnetPayment.settings.tariff', $order->getSalesChannelId())
        ];

        $parameters['customer'] = $this->bookAmountRequest($order, $context);
        ...
        $paymentType = ($transactionData->getPaymentType() == 'novalnetsepa') ? 'DIRECT_DEBIT_SEPA' : (($transactionData->getPaymentType() == 'novalnetdirectdebitach') ? 'DIRECT_DEBIT_ACH' : strtoupper(str_replace('novalnet', '', $transactionData->getPaymentType())));
        ...
        $parameters['transaction'] = [
            ...
            'test_mode'      => (int) $this->helper->getNovalnetPaymentSettings("NovalnetPayment.settings." . $paymentCode . "TestMode", $order->getSalesChannelId()),
        ...
        // Built payment details
        if (!is_null($transactionData->getAdditionalDetails())) {
            $additionalDetails = $this->helper->unserializeData($transactionData->getAdditionalDetails());
            $parameters['transaction']['payment_data']['token'] = $additionalDetails['token'];
        }

        $response = $this->helper->sendPostRequest($parameters, $this->helper->getActionEndpoint('payment'), $this->helper->getNovalnetPaymentSettings('NovalnetPayment.settings.accessKey', $order->getSalesChannelId()));
    ...
    public function refundTransaction(NovalnetPaymentTransactionEntity $transactionData, OrderTransactionEntity $transaction, Context $context, int $refundAmount, Request $request): array
    {
        ...
        if ($this->validator->isSuccessStatus($response)) {
            if (! empty($response['transaction']['status'])) {
                ...
                } elseif ($transactionData->getGatewayStatus() != 'PENDING' && !preg_match('/instalment/', $transactionData->getPaymentType())) {
        ...
        if (!empty($instalmentData['cycle_dates'])) {
            ...
            foreach ($futureInstalmentDates as $cycle => $futureInstalmentDate) {
                ...
                if ($cycle == count($instalmentData['cycle_dates'])) {
                    $amount = $instalmentData ['total_amount'] - ($instalmentData['cycle_amount'] * ($cycle - 1));
     ...
     * @return array
     */
    public function getNovalnetInstalmentInfo(Context $context, $orderNumber): array
    {
        $transactionData = $this->fetchNovalnetTransactionData((string) $orderNumber, $context);

        if (!empty($transactionData) && $transactionData->getGatewayStatus() === 'CONFIRMED' && !empty($transactionData->getAdditionalDetails()) && false !== strpos($transactionData->getAdditionalDetails(), 'InstalmentDetails')) {
        ...
        foreach ($instalmentDetails as $key => $values) {
            if ($values['reference'] == $referenceTid) {
                ...
                if ($instalmentDetails[$key]['refundAmount'] >= $values['amount']) {
                    ...
                    $instalmentDetails[$key]['status'] = $this->translator->trans('NovalnetPayment.text.refundPartialMsg', [], null, $locale);
                }
            }
        }
        return $instalmentDetails;
    ...
    public function getOrderTransactionDetails(string $transactionId, Context $context): ?OrderTransactionEntity
    {
        ...
        $orderCriteria->addAssociation('order.currency');
