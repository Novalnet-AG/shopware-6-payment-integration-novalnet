...
class NovalnetOrderTransactionHelper
{
    ...
    */
    ...
    public function prepareMailContent(OrderEntity $order, SalesChannelContext $salesChannelContext, string $note, $instalmentRecurring = false): void
    {
        if (!empty($order->getOrderCustomer())) {
            ...
            try {
                $emailConfigs = $this->helper->getNovalnetPaymentSettings('NovalnetPayment.settings.emailMode', $salesChannelContext->getSalesChannel()->getId());
                if (!empty($emailConfigs) && $emailConfigs == 1) {
    ...
    */
        ...
        if (!empty($instalmentData['cycle_dates'])) {
            ...
            foreach (array_keys($futureInstalmentDates) as $cycle) {
                $additionalDetails['InstalmentDetails'][$cycle] = [
                    ...
                    'cycleDate'     => !empty($futureInstalmentDates[$cycle + 1]) ? date('Y-m-d', strtotime($futureInstalmentDates[$cycle + 1])) : '',
                    'cycleExecuted' => '',
                    'dueCycles'     => '',
                    'paidDate'      => '',
     ...
     * @return array
     */
    public function getSubscriptionDetails(Context $context, string $orderNumber, string $locale): array
    {
        $criteria = new criteria();
        $criteria->addFilter(new EqualsFilter('novalnet_transaction_details.orderNo', $orderNumber));
        ...
        $datas = [
            'payment_details' => [
                    ...
                    'process_mode' => 'direct',
                    'name' => (!empty($subscription['subscription']) && isset($subscription['subscription']['payment_details']['name'])) ? $subscription['subscription']['payment_details']['name'] : $this->helper->getUpdatedPaymentName($orderDetails->getPaymentType(), $locale),
            ],
            'booking_details' => [
    ...
    /**
     * Update Subscription Data for change payment method.
     *
     * @param string $status
    ...
    public function updateChangePayment(array $input, string $orderId, Context $context, bool $ordeUpdatedPayment = false)
    {
        ...
        if (isset($input['event']) && isset($input['event']['type'])) {
            ...
            if ($schemaManager->tablesExist(array('novalnet_subscription')) == true) {
                $data = $connection->fetchOne(sprintf("SELECT `id` FROM `novalnet_subscription` WHERE `order_id` = UNHEX('%s')", $orderId));
                $subCycleData = $connection->fetchAssociative(sprintf("SELECT * FROM `novalnet_subs_cycle` WHERE `order_id` = UNHEX('%s')", $orderId));
                ...
                if (!empty($subCycleData) && isset($input['custom']['input2']) && $input['custom']['input2'] == 'subParentOrderNumber') {
                    ...
                    $subscriptiondata = $connection->fetchAssociative(sprintf("SELECT * FROM `novalnet_subscription` WHERE `id` = UNHEX('%s')", $input['custom']['inputval4']));
                    $subscriptionDate = strtotime($subscriptiondata['next_date']);
                    $formatDate = date('Y-m-d H:i:s', $subscriptionDate);
                    $nextDate = $this->getUpdatedNextDate((int) $subscriptiondata['interval'], $this->getSubscriptionUnitValue($subscriptiondata['unit']), $formatDate, $subscriptiondata, $input ['custom']['inputval4']);

                    $connection->exec(sprintf("UPDATE `novalnet_subscription` SET `status` = 'ACTIVE', next_date = '%s', payment_method_id = UNHEX('%s') WHERE `id` = UNHEX('%s')", $nextDate, $input ['custom']['paymentMethodId'], $input['custom']['inputval4']));

                    if ((($subscriptiondata['length'] > $subCycleData['cycles']) || ($subscriptiondata['length'] == 0))) {
                        $id = Uuid::randomHex();
                        $cycleDate = $this->getFormattedDate((int) $subscriptiondata['interval'], $this->getSubscriptionUnitValue($subscriptiondata['unit']), date('Y-m-d H:i:s'));
                        $connection->exec(sprintf("INSERT INTO `novalnet_subs_cycle` (`id`, `subs_id`, `order_id`, `amount`, `interval`, `period`, `payment_method_id`, `cycles`, `cycle_date`, `status`) VALUES(UNHEX('%s'),UNHEX('%s'),null,'%s','%s','%s',UNHEX('%s'),'%s','%s','%s')", $id, $input['custom']['inputval4'], $subCycleData['amount'], $subCycleData['interval'], $subCycleData['period'], $input ['custom']['paymentMethodId'], $subCycleData['cycles'] + 1, $cycleDate, 'PENDING'));

                    }

                    if (($subscriptiondata['length'] == $subCycleData['cycles'])) {
                        $connection->exec(sprintf("UPDATE `novalnet_subscription` SET `status` = 'PENDING_CANCEL', payment_method_id = UNHEX('%s') WHERE `id` = UNHEX('%s')", $input ['custom']['paymentMethodId'], $input ['custom']['inputval4']));
                    }
     ...
     * @return string
     */
    public function getUpdatedNextDate(int $interval, string $period, $date, array $subscription, string $id): string
    {
        if (!empty($subscription) && !empty($subscription['last_day_month']) && $period == 'months') {
    ...
    public function subscriptionStatusUpdate(string $orderId, Context $context, string $locale)
    {
        ...
        if ($schemaManager->tablesExist(array('novalnet_subscription')) == true) {
            $subCycleDatas = $connection->fetchAllAssociative(sprintf("SELECT * FROM `novalnet_subs_cycle` WHERE `order_id` = UNHEX('%s')", $orderId));

            if (!empty($subCycleDatas)) {
                foreach ($subCycleDatas as $subCycleData) {
                    $subId = $subCycleData['subs_id'];
                    $cancelReason = sprintf($this->translator->trans('NovalnetPayment.text.subscriptionChargeback', [], null, $locale), date('Y-m-d H:i:s'));
                    $connection->exec(sprintf("UPDATE `novalnet_subs_cycle` SET `status` = 'FAILURE' WHERE `order_id` = UNHEX('%s')", $orderId));
                    $connection->exec(sprintf("UPDATE `novalnet_subscription` SET `status` = 'CANCELLED', `cancel_reason`='%s', `cancelled_at` = '%s' WHERE `id` ='%s'", $cancelReason, date('Y-m-d H:i:s'), $subId));
                }
            }
        }
    }
    ...
    {
        return ($aboUnit == 'd') ? 'days' : ($aboUnit == 'w' ? 'weeks' : ($aboUnit == 'm' ? 'months' : 'years'));
    }
