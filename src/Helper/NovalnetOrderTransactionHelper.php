...
class NovalnetOrderTransactionHelper
{
    ...
    */
     ...
     * @return array
     */
    public function getSubscriptionDetails(Context $context, string $orderNumber, string $orderId): array
    {
        $transactionDetails = $this->fetchNovalnetTransactionData($orderNumber, $context, null, true);
        $subscription = $this->helper->unserializeData($transactionDetails->getAdditionalDetails());
        $locale = $this->helper->getLocaleFromOrder($orderId);
        $datas = [
            'payment_details' => [
                    'type' => (!empty($subscription['subscription']) && isset($subscription['subscription']['payment_details']['type'])) ? $subscription['subscription']['payment_details']['type'] : $this->helper->getUpdatedPaymentType($transactionDetails->getPaymentType()),
                    'process_mode' => 'direct',
                    'name' => (!empty($subscription['subscription']) && isset($subscription['subscription']['payment_details']['name'])) ? $subscription['subscription']['payment_details']['name'] : $this->helper->getUpdatedPaymentName($transactionDetails->getPaymentType(), $locale),
            ],
            'booking_details' => [
    ...
    public function updateSubscriptionData(string $status, string $paymentMethodId, string $orderId)
    {
        ...
        if (!empty($novalnetSubsriptionTableExist)) {
            $connection = $this->container->get(Connection::class);
                ...
                if (!empty($subCycleData) && isset($input['custom']['input2']) && $input['custom']['input2'] == 'subParentOrderNumber') {
                    ...
                    if (!empty($subOrdersLast->getOrderId()) && $subOrdersLast->getStatus() !== 'PENDING' && ($subscriptiondata->getLength() == 0 || $subscriptiondata->getLength() > $count)) {
                        $subData = [
                            ...
                            'status' => 'PENDING',
                            'cycles' => $count + 1,
                            'cycleDate' => date('Y-m-d H:i:s', strtotime('+ '. $subscriptiondata->getInterval() . $unit, strtotime($subscriptiondata->getNextDate()->format('Y-m-d H:i:s'))))
                        ];
                        $this->container->get('novalnet_subs_cycle.repository')->upsert([$subData], $context);
                    }
    ...
    public function subscriptionStatusUpdate(string $orderId, Context $context, string $locale)
    {
        ...
        if (($this->container->has('novalnet_subscription.repository')) && !empty($novalnetSubsriptionTableExist)) {
            $criteria = new criteria();
            $criteria->addFilter(new EqualsFilter('orderId', $orderId));
            $criteria->addAssociation('novalnetSubscription');
    ...
    public function getOrderTransactionDetails(string $transactionId): ?OrderTransactionEntity
    {
        ...
        $orderTransaction  = $this->orderTransactionRepository->search($orderCriteria, Context::createDefaultContext())->first();
    ...
    public function getSalesChannelDetails(OrderEntity $order): ?SalesChannelContext
    {
        ...
        if (!empty($billingAddress)) {
            $billingAddressId = $this->helper->getAddressId($billingAddress) ?? $order->getBillingAddressId();
        }
