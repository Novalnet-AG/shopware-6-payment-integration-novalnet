        ...
        if ($type) {
            ...
            if ($this->validator->isSuccessStatus($response)) {
                ...
                if (! empty($response['transaction']['status'])) {
                    ...
                    if (in_array($transactionData->getPaymentType(), ['novalnetinvoice', 'novalnetinvoiceguarantee', 'novalnetinvoiceinstalment', 'novalnetsepainstalment', 'novalnetsepaguarantee']) && in_array($response['transaction']['status'], ['CONFIRMED', 'PENDING'])) {
						$order = $this->helper->getOrderCriteria($transaction->getOrderId(), $context);
						$options = [
						...
						$salesChannelContext = $this->helper->getSalesChannelContext($order->getSalesChannelId(), $options);
