                ...
                if (in_array($status, ['ON_HOLD',  'PENDING',  'CONFIRMED'])) {
                    ...
                    if (!empty($subscriptionData)) {
                        ...
                        if (!empty($subscriptionData['length'])) {
                            $connection->exec(sprintf('UPDATE `novalnet_subscription` SET `status` = \'ACTIVE\', next_date = \'%s\', ending_at = \'%s\', payment_method_id = UNHEX(\'%s\') WHERE `order_id` = UNHEX(\'%s\')', $nextDate, $endingDate, $paymentMethodId, $orderId));
                        } else {
                            $connection->exec(sprintf('UPDATE `novalnet_subscription` SET `status` = \'ACTIVE\', next_date = \'%s\', payment_method_id = UNHEX(\'%s\') WHERE `order_id` = UNHEX(\'%s\')', $nextDate, $paymentMethodId, $orderId));
                        }
