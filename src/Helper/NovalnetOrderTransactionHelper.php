    ...
    */
    ...
    public function getQrImage(Context $context, string $orderNumber): string
    {
        ...
        if (!empty($transactionData)) {
            if (in_array($transactionData->getPaymentType(), ['PREPAYMENT', 'INVOICE', 'GUARANTEED_INVOICE', 'INSTALMENT_INVOICE'])) {
        ...
        if ($status) {
            ...
            if ($this->helper->isSuccessStatus($response)) {
                ...
                if (!empty($response['transaction']['status'])) {
                    ...
                    if (in_array($paymentType, ['INVOICE', 'GUARANTEED_INVOICE', 'INSTALMENT_INVOICE', 'INSTALMENT_DIRECT_DEBIT_SEPA'], true) && in_array($response['transaction']['status'], ['CONFIRMED', 'PENDING'])) {
