...
class NovalnetHelper
{
   ...
   /**
    ...
    {
        return $this->shopVersion . '-NN' . '13.0.0';
    }
    ...
    public function formBankDetails(array $input, Context $context, string $languageId = null) : string
    {
        ...
        if (!empty($input ['transaction']['amount']) && $input ['transaction']['amount']== 0 && in_array($paymentType, ['PREPAYMENT', 'INVOICE'])) {
            $comments .= '';
            ...
            if (!empty($input['transaction']['status']) && $input['transaction']['status'] === 'PENDING' &&
                (preg_match('/INVOICE/', $paymentType) && (preg_match('/GUARANTEED/', $paymentType) || preg_match('/INSTALMENT/', $paymentType)))
    ...
    public function formOrderComments(array $input, Context $context, string $languageId = null) : string
    {
        ...
        if (!empty($input ['transaction']['tid'])) {
            ...
            if (!empty($input ['transaction']['test_mode']) &&  $input ['transaction']['test_mode'] == 1) {
        ...
        if (!empty($input['transaction']['status']) && $input['transaction']['status'] === 'CONFIRMED' && $input['transaction']['amount'] == 0 && in_array($paymentType, ['CREDITCARD', 'DIRECT_DEBIT_SEPA']) && (!empty($paymentdata['booking_details']['payment_action']) && $paymentdata['booking_details']['payment_action'] == 'zero_amount')) {
    ...
    {
        $this->container->get('novalnet_transaction_details.repository')->update([$data], $context);
    }
