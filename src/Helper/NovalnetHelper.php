...
class NovalnetHelper
{
   ...
   /**
    ...
    public function formBankDetails(array $input, Context $context, string $languageId = null) : string
    {
        ...
        if ($input ['transaction']['amount'] == 0 && in_array($paymentType, ['PREPAYMENT', 'INVOICE'])) {
            $comments .= '';
        } elseif ($input['transaction']['status'] != 'DEACTIVATED') {
    ...
    public function formOrderComments(array $input, Context $context, string $languageId = null) : string
    {
        ...
        if ($input['transaction']['status'] === 'CONFIRMED' && $input['transaction']['amount'] == 0 && in_array($paymentType, ['CREDITCARD', 'DIRECT_DEBIT_SEPA']) && (!empty($paymentdata['booking_details']['payment_action']) && $paymentdata['booking_details']['payment_action'] == 'zero_amount')) {
