...
class NovalnetHelper
{
   ...
   /**
    ...
    {
        return $this->shopVersion . '-NN' . '13.4.3';
    }
    ...
    public function getNovalnetIframeUrl(SalesChannelContext $salesChannelContext, $transaction, bool $subscription = false) : string
    {
        ...
        $response = $this->getNovalnetIframeResponse($salesChannelContext->getSaleschannel()->getId(), $salesChannelContext->getCustomer()->getId(), $requiredFields, $salesChannelContext->getContext(), 'seamless_payment', $subscriptionOrder);
    ...
    {
        $paymentSettings =  $this->systemConfigService->getDomain(
            'NovalnetPayment.settings',
            $salesChannelId,
            true
        );
        
        if (!empty($paymentSettings['NovalnetPayment.settings.clientId']))
        {
			$paymentSettings['NovalnetPayment.settings.clientId'] = str_replace(' ', '', $paymentSettings['NovalnetPayment.settings.clientId']);
		}
    ...
    public function getCustomerData(CustomerEntity $customerEntity)
    {
        ...
        if(!empty($this->getCurrentRequest())){
			$requested = $this->getCurrentRequest();
    ...
    public function getIp(string $type = 'REMOTE') : string
    {
        ...
        if ($type === 'REMOTE') {
            $ipAddress = !empty($currentRequest) ? $currentRequest->getClientIp() : '';
        } else {
    ...
    public function formBankDetails(array $input, Context $context, string $languageId = null) : string
    {
		...
		} elseif (!empty($input['transaction']['amount'])) {
			$amountInBiggerCurrencyUnit = $this->amountInBiggerCurrencyUnit($input['transaction']['amount'], $input ['transaction']['currency']);
		}
                
        if (!empty($input['transaction']['status']) && $input['transaction']['status'] != 'DEACTIVATED' && $input ['transaction']['amount'] && $input ['transaction']['amount'] > 0) {
            if ($input['transaction']['status'] === 'PENDING' && in_array($paymentType, ['GUARANTEED_INVOICE', 'INSTALMENT_INVOICE'])) {
                ...
                if (!empty($amountInBiggerCurrencyUnit)) {
                    if (in_array($input['transaction']['status'], [ 'CONFIRMED', 'PENDING' ], true) && ! empty($input ['transaction']['due_date'])) {
                        $comments .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.amountTransaferNoteWithDueDate', [], null, $localeCode), $amountInBiggerCurrencyUnit, date('d/m/Y', strtotime($input ['transaction']['due_date']))) . $this->newLine;
                    } else {
                        $comments .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.amountTransaferNote', [], null, $localeCode), $amountInBiggerCurrencyUnit) . $this->newLine;
                    }
                }
                
                $comments .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.accountHolder', [], null, $localeCode), $bankDetails['account_holder']);
                $comments .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.bank', [], null, $localeCode), $bankDetails['bank_name']);
                $comments .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.bankPlace', [], null, $localeCode), $bankDetails['bank_place']);
                $comments .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.iban', [], null, $localeCode), $bankDetails['iban']);
                $comments .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.bic', [], null, $localeCode), $bankDetails['bic']) . $this->newLine;

                if (! empty($input ['transaction']['invoice_ref'])) {
					 // Form reference comments.
					$comments .= $this->newLine . $this->translator->trans('NovalnetPayment.text.paymentReferenceNoteAny', [], null, $localeCode). $this->newLine;
					$comments .=  sprintf($this->translator->trans('NovalnetPayment.text.paymentReference', [], null, $localeCode), '1', $input ['transaction']['tid']);
					...
					$comments .=  sprintf($this->translator->trans('NovalnetPayment.text.paymentReference', [], null, $localeCode), '', $input ['transaction']['tid']);
				}
            }
            ...
            if (!empty($input['transaction']['partner_payment_reference']) && !empty($amountInBiggerCurrencyUnit)) {
                $comments .= $this->newLine . $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.multibancoReference', [], null, $localeCode), $amountInBiggerCurrencyUnit);
                $comments .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.paymentReference', [], null, $localeCode), '', $input['transaction']['partner_payment_reference']);
                if (!empty($input['transaction']['service_supplier_id'])) {
					$comments .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.paymentEntityReference', [], null, $localeCode), $input['transaction']['service_supplier_id']);
				}
            }
        }
        return $comments;
    ...
    public function formOrderComments(array $input, Context $context, string $languageId = null) : string
    {
        ...
        if (!empty($input['custom']['input3']) && $input['custom']['input3'] == 'ZeroBooking') {
     ...
     * @return array
     */
    public function getNovalnetIframeResponse(string $saleschannelId, string $customerId, array $requiredFields, Context $context, string $endPoint, string $type = '') :array 
    {
		$paymentSettings = $this->getNovalnetPaymentSettings($saleschannelId);
		
        $parameters['merchant'] = $this->merchantParameter($paymentSettings);

        $customer = $this->getCustomerDetails($customerId, $context);
        ...
        return $this->sendPostRequest($parameters, $this->getActionEndpoint($endPoint), str_replace(' ', '', $paymentSettings['NovalnetPayment.settings.accessKey']));
	}
    ...
    public function checkWebhookIp(string $novalnetHostIp): bool
    {
		...
		$getCurrentRequest = $this->getCurrentRequest();
	...
	public function merchantParameter(array $paymentSettings): array
    {
		$parameters = [
            'signature' => $paymentSettings['NovalnetPayment.settings.clientId'],
            'tariff'    => $paymentSettings['NovalnetPayment.settings.tariff']
