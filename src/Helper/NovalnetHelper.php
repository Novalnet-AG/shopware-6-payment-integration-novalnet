...
class NovalnetHelper
{
    ...
    public function getVersionInfo(Context $context) : string
    {
        ...
        return $this->shopVersion . '-NN' . '12.2.1';
    }
    ...
    public function formBankDetails(array $input) : string
    {
        ...
        if ($input['transaction']['status'] === 'PENDING' && in_array($input['transaction']['payment_type'],['GUARANTEED_INVOICE', 'INSTALMENT_INVOICE'])) {
            ...
            $comments .= $this->newLine . $this->translator->trans('NovalnetPayment.text.paymentReferenceNote'). $this->newLine;

            if ($input['transaction']['payment_type'] !== 'INSTALMENT_INVOICE') {
				
				/* translators: %s:  TID */
                $comments .=  sprintf($this->translator->trans('NovalnetPayment.text.paymentReference'), '1', 'TID '. $input ['transaction']['tid']);
                    
                /* translators: %s: invoice_ref */
                if(! empty($input ['transaction']['invoice_ref']))
                {
					$comments .=  $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.paymentReference'), '2', $input ['transaction']['invoice_ref']) . $this->newLine;
                }
