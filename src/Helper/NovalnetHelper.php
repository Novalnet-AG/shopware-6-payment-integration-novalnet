...
class NovalnetHelper
{
    ...
    {
        return $this->shopVersion . '-NN' . '12.6.0';
    }
    ...
    public function formBasicComments(array $input, Context $context, string $languageId = null): string
    {
        ...
        if (in_array($input ['transaction']['payment_type'], ['CREDITCARD' ,'DIRECT_DEBIT_SEPA','GOOGLEPAY', 'APPLEPAY']) && (isset($input['transaction']['status']) && $input['transaction']['status'] === 'CONFIRMED') && (isset($input['transaction']['amount']) && $input['transaction']['amount'] == 0)) {
