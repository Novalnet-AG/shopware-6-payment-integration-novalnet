...
class NovalnetHelper
{
    ...
    {
        return $this->shopVersion . '-NN' . '12.9.2';
    }
    ...
    public function formBasicComments(array $input, Context $context, ?string $languageId = null): string
    {
        ...
        if (in_array($input ['transaction']['payment_type'], ['CREDITCARD' ,'DIRECT_DEBIT_SEPA','GOOGLEPAY', 'APPLEPAY', 'DIRECT_DEBIT_ACH']) && (isset($input['transaction']['status']) && $input['transaction']['status'] === 'CONFIRMED') && (isset($input['transaction']['amount']) && $input['transaction']['amount'] == 0)) {
            if (isset($input['custom']) && isset($input['custom']['input4']) && $input['custom']['input4'] == 'ZeroBooking') {
    ...
    public function formBankDetails(array $input, Context $context, ?string $languageId = null): string
    {
        ...
        if (isset($input['transaction']['amount']) && $input['transaction']['amount'] == 0 && in_array($input['transaction']['payment_type'], ['PREPAYMENT', 'INVOICE'])) {
            $comments .= '';
     ...
     */
    public function getOrderCriteria(Context $context, ?string $orderId = null, ?string $customerId = null): ?OrderEntity
    {
        if (!empty($orderId)) {
            $orderCriteria = new Criteria([$orderId]);
        } else {
        ...
        if ($schemaManager->tablesExist(array('novalnet_subscription')) == true) {
            ...
            if (!empty($data)) {
                if (in_array($status, ['ON_HOLD',  'PENDING',  'CONFIRMED'])) {
                    $connection->executeQuery(sprintf("UPDATE `novalnet_subscription` SET `status` = 'ACTIVE', payment_method_id = UNHEX('%s') WHERE `order_id` = UNHEX('%s')", $paymentId, $orderId));
                    $connection->executeQuery(sprintf("UPDATE `novalnet_subs_cycle` SET `status` = 'SUCCESS', payment_method_id = UNHEX('%s') WHERE `order_id` = UNHEX('%s')", $paymentId, $orderId));
                } else {
                    $connection->executeQuery(sprintf("UPDATE `novalnet_subscription` SET `status` = 'CANCELLED', cancel_reason = 'Parent order getting failed', cancelled_at = '%s', payment_method_id = UNHEX('%s') WHERE `order_id` = UNHEX('%s')", date('Y-m-d H:i:s'), $paymentId, $orderId));
                    $connection->executeQuery(sprintf("UPDATE `novalnet_subs_cycle` SET `status` = 'FAILURE', payment_method_id = UNHEX('%s') WHERE `order_id` = UNHEX('%s')", $paymentId, $orderId));
                }
            }
        }
    }
    ...
    public function setStatus(string $transactionId, string $action, Context $context): void
    {
        try {
            ...
            if (!empty($status)) {
                $connection = $this->container->get(Connection::class);
                $connection->executeQuery(sprintf("UPDATE `order_transaction` SET `state_id` = UNHEX('%s') WHERE `id` = UNHEX('%s');", $status->getId(), $transactionId));
            }
    ...
    public function updateChangePayment(array $input, string $orderId, Context $context, bool $ordeUpdatedPayment = false)
    {
        ...
        if (!empty($ordeUpdatedPayment)) {
            ...
            if (isset($input['event']) && isset($input['event']['type'])) {
                if ($schemaManager->tablesExist(array('novalnet_subscription')) == true && $this->container->has('novalnet_subscription.repository')) {
                    $criteria = new criteria();
    ...
    public function getUpdatedNextDate($subscription): string
    {
        ...
        if (!empty($subscription->getLastDayMonth()) && $unit == 'months') {
            ...
            if (date("d", strtotime($date)) == date("d", strtotime($date . "+$interval month"))) {
                ...
                $this->container->get(Connection::class)->executeQuery(sprintf("UPDATE `novalnet_subscription` SET `last_day_month` = '%s' WHERE `id` = UNHEX('%s')", 1, $subscription->getId()));
    ...
    public function getSalesChannelDetails(OrderEntity $order): ?SalesChannelContext
    {
        $customerId = $order->getOrderCustomer()->getCustomerId();
        $currencyId = $order->getCurrencyId() ?: $order->getCurrency()->getId();
        $options = [
        ...
        $customerAddresses = $this->container
            ->get('customer_address.repository')
        ...
        if ($billingAddress) {
            $billingAddressId = $this->getAddressId($billingAddress) ?? $order->getBillingAddressId();
        }
        ...
        if (!empty($shippingAddress)) {
            $shippingAddressId = $this->getAddressId($shippingAddress);
        } else {
        ...
        $salesChannelContext = $this->container
            ->get(SalesChannelContextFactory::class)
            ->create(Uuid::randomHex(), $order->getSalesChannelId(), $options);

        return $salesChannelContext;
    }

    public function getSalesChannelPaymentMethods($salesChannelContext, Context $context)
    {
        $criteria = new Criteria();
        ...
        $criteria->addFilter(
            new PrefixFilter('handlerIdentifier', 'Novalnet')
        );
        $paymentMethods = $this->container->get('payment_method.repository')->search($criteria, $context);
