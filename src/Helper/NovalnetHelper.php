...
class NovalnetHelper
{
    ...
    {
        return $this->shopVersion . '-NN' . '12.4.0';
    }
     ...
     * @return string
     */
    public function formBasicComments(array $input, Context $context) : string
    {
        $comments = '';
        $localeCode = $this->getShopLocale($context);
        
        if ((! empty($input['result']['status']) && 'FAILURE' === $input['result']['status'])) {
			$comments .= $input ['result']['status_text'] . $this->newLine;
		}
    ...
    public function getNovalnetErrorMessage(string $transactionId, string $saleschannelId) : ?string 
    {
        ...
        } elseif($this->contextPersister->load($transactionId, $saleschannelId)) {
     ...
     * @return string|null
     */
    public function getApplePayPaymentId(Context $context) : ?string 
    {
		$criteria = new Criteria();
        $criteria->addFilter(new EqualsFilter('handlerIdentifier', NovalnetApplePay::class));
     ...
     * 
     */
    public function getShippingCosts($shippingId, $context)
    {
		$criteria = new Criteria([$shippingId]);
		$criteria->addAssociation('prices');
		$shipping = $this->container->get('shipping_method.repository')->search($criteria, $context)->first();
		return $shipping->getPrices()->first()->getCurrencyPrice();
	}
}
