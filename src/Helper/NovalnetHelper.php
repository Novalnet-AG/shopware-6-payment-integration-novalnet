...
class NovalnetHelper
{
    ...
    public function getPaymentMethodName(PaymentMethodEntity $paymentMethodEntity): ?string
    {
        $paymentMethodName = '';
		$paymentMethodName = '';
		if ($paymentMethodEntity->getShortName() !== null) {
     ...
     * @return float
     */
    public function amountInLowerCurrencyUnit($amount): ?float
    {
        return  $amount * 100;
    }
     ...
     * @return string
     */
    public function formBasicComments(array $input, $isError = false) : string
    {
        $comments = '';
        ...
        if (! empty($input ['transaction']['tid'])) {
            ...
            if ($input['transaction']['status'] === 'PENDING' && $input['transaction']['payment_type'] === 'GUARANTEED_DIRECT_DEBIT_SEPA') {
    ...
    public function formBankDetails(array $input) : string
    {
        ...
        if ($input['transaction']['status'] === 'PENDING' && $input['transaction']['payment_type'] === 'GUARANTEED_INVOICE') {
            ...
            if (in_array($input['transaction']['status'], [ 'CONFIRMED', 'PENDING' ], true) && ! empty($input ['transaction']['due_date'])) {
                $comments .= $this->newLine . $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.amountTransaferNoteWithDueDate'), $amountInBiggerCurrencyUnit, date('d/m/Y', strtotime($input ['transaction']['due_date']))) . $this->newLine . $this->newLine;
            } else {
                $comments .= $this->newLine . $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.amountTransaferNote'), $amountInBiggerCurrencyUnit) . $this->newLine . $this->newLine;
            }
            ...
            /* translators: %s:  TID */
            $comments .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.paymentReference'), '1', $input ['transaction']['tid']);
    
            if (! empty($input ['transaction']['invoice_ref'])) {
                /* translators: %s: invoice_ref */
                $comments .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.paymentReference'), '2', $input ['transaction']['invoice_ref']);
            }
        }
    ...
    public function formSlipDetails(array $input) : string
    {
        ...
        if (! empty($input ['transaction']['due_date'])) {
            ...
            $comments .= $this->newLine . $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.slipExpiryDate'), date('d/m/Y', strtotime($input ['transaction']['due_date']))) . $this->newLine . $this->newLine;
        }
     ...
     * @return string
     */
    public function getNovalnetErrorMessage(SalesChannelContext $salesChannelContext) : string
    {
        $errorMessage = '';
