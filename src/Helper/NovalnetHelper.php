...
class NovalnetHelper
{
    ...
    {
        return $this->shopVersion . '-NN13.6.2';
    }
    ...
    public function getNovalnetIframeUrl(SalesChannelContext $salesChannelContext, $transaction, string $type = ''): string
    {
        ...
        if (method_exists($transaction, 'getOrder')) {
            ...
            $order = $this->container->get('order.repository')->search($criteria, $salesChannelContext->getContext())->first();

            if (!is_null($order)) {
                $salesChannelContext = $this->getSalesChannelDetails($order);
            }
        ...
        $requiredFields = ['amount' => $amount, 'currency' => $salesChannelContext->getCurrency()->getIsoCode() ?: $salesChannelContext->getSalesChannel()->getCurrency()->getIsoCode()];
    ...
    public function getThemeName(string $saleschannelId, Context $context): string
    {
        ...
        foreach ($themecollection as $key => $data) {
            if ($data->isActive() == 1) {
                $themeTechnicalName = $data->getTechnicalName() ?? str_replace(' ', '', $data->getName());
                break;
            }
        }
     ...
     *
     * Retrieves the sales channel context for a given order.
     * The context is created based on the order's customer, currency, billing and shipping addresses.
     * If the order does not have a shipping address, the billing address is used instead.
     * If the order does not have a billing address, the context is not created and the method returns null.
     */
    ...
    public function getCustomerData(CustomerEntity $customerEntity)
    {
        $customer = [];
        // Get billing details.
        [$billingCustomer, $billingAddress] = $this->getAddress($customerEntity, 'billing');
        ...
        [$shippingCustomer, $shippingAddress] = $this->getAddress($customerEntity, 'shipping');
    ...
    public function getLocaleCodeFromContext(Context $context, bool $formattedLocale = false, ?string $languageId = null): string
    {
        ...
        if (!$formattedLocale) {
            ...
            $lang = explode('-', (string) $locale->getCode());
        ...
        if (isset($input['transaction']['amount']) && $input['transaction']['amount'] === 0 && in_array($paymentType, ['PREPAYMENT', 'INVOICE'])) {
            ...
            if ($input['transaction']['status'] === 'PENDING' && in_array($paymentType, ['GUARANTEED_INVOICE', 'INSTALMENT_INVOICE'])
                ...
                if (isset($input['instalment']) && isset($input['instalment']['cycle_amount']) && $input['instalment']['cycle_amount'] !== 0) {
                    $amountInBiggerCurrencyUnit = $this->amountInBiggerCurrencyUnit($input['instalment']['cycle_amount'], $input['instalment']['currency'] ?? $input['transaction']['currency']);
                ...
                if (!empty($amountInBiggerCurrencyUnit)) {
                    if (in_array($input['transaction']['status'], ['CONFIRMED', 'PENDING']) && !empty($input['transaction']['due_date'])) {
                        $comments .= $this->newLine . sprintf($translator->trans('NovalnetPayment.text.amountTransaferNoteWithDueDate', [], null, $localeCode), $amountInBiggerCurrencyUnit, date('d/m/Y', strtotime((string) $input['transaction']['due_date']))) . $this->newLine;
                    } else {
                ...
                $comments .= $this->newLine . sprintf($translator->trans('NovalnetPayment.text.bic', [], null, $localeCode), $bankDetails['bic']);
                $comments .= $this->newLine . sprintf($translator->trans('NovalnetPayment.text.bank', [], null, $localeCode), $bankDetails['bank_name']);
                $comments .= $this->newLine . sprintf($translator->trans('NovalnetPayment.text.bankPlace', [], null, $localeCode), $bankDetails['bank_place']) . $this->newLine;
    ...
    public function getNovalnetIframeResponse(string $saleschannelId, CustomerEntity $customer, array $requiredFields, Context $context, string $type = ''): array
    {
        ...
        if (!empty($server) && !empty($server['REQUEST_URI']) && preg_match('/account\/payment/', (string) $server['REQUEST_URI'])) {
    ...
    public function getNovalnetRequestData(float $amount, string $orderNumber, array $data, SalesChannelContext $salesChannelContext): array
    {
        ...
        $parameters['transaction'] = [
            ...
            'currency' => $salesChannelContext->getCurrency()->getIsoCode() ?: $salesChannelContext->getSalesChannel()->getCurrency()->getIsoCode(),
        ...
        foreach ($ipKeys as $key) {
            if (array_key_exists($key, $_SERVER) === true) {
                if (in_array($key, ['HTTP_X_FORWARDED_HOST', 'HTTP_X_FORWARDED_FOR'])) {
                    $forwardedIps = (!empty($_SERVER[$key])) ? explode(',', (string) $_SERVER[$key]) : [];
