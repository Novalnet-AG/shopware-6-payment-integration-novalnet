...
class NovalnetPaymentTokenRepository
{
     ...
     * @param array $data
     */
    public function savePaymentToken(string $customerId, Context $context, array $data): void
    {
        $result = null;
		$card = $this->getExistingPaymentToken($customerId, $context, $data);
		if ($card === null) {
		...
		$this->tokenRepository->upsert([$data], $context);
     ...
     */
    public function getExistingPaymentToken(string $customerId, Context $context, array $data): ?NovalnetPaymentTokenEntity
    {
		...
		$criteria->addFilter(
			new EqualsFilter('novalnet_payment_token.customerId', $customerId)
		);
    ...
    {
        $card = $this->getExistingPaymentToken($salesChannelContext->getCustomer()->getId(), $salesChannelContext->getContext(), $data);
