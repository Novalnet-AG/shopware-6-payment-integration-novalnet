...
class PaymentMethodInstaller
{
    ...
    */
    ...
    private function addPaymentMethods(): void
    {
        ...
        if (!$customFieldExistsId && !empty($this->container->has('custom_field_set.repository'))) {
        ...
        if (!$customFieldsPaymentExistsId && !empty($this->container->has('custom_field_set.repository'))) {
                ...
                if ($payment->gethandlerIdentifier() != 'Novalnet\NovalnetPayment\Service\NovalnetPayment') {
                    ...
                    if ($getSalessChannels != null) {
                        foreach ($channels->getIds() as $channel) {
                            ...
                            $criteria = new Criteria();
                            $criteria->addFilter(new EqualsFilter('salesChannelId', $channel));
                            // Fetch the payment methods for the specified sales channel
                            $salesChannelPaymnetMethod = $getSalessChannels->searchIds($criteria, $this->context);
                            if($salesChannelPaymnetMethod != null) {
                                $data = [
                                    'salesChannelId'  => $channel,
    ...
    public function alterPaymentTokenTable(): void
    {
        ...
        $isTableExists  = $connection->executeQuery('
			...
			AND table_schema = database()')->fetch();
						...
						<tbody>
							{% for info in instalmentInfo.InstalmentDetails %}
								...
								<tr>
									...
									<td style="border-bottom:1px solid #cccccc;">{{ info.cycleDate ? info.cycleDate|date("Y-m-d"): "-" }}</td>
								<tr>
							{% endfor %}
						</tbody>
					</table>
    ...
    private function getPlainTemplateEn(): string
    {
		...
		{% if "INSTALMENT_INVOICE" in novalnetDetails.paymentType or "INSTALMENT_DIRECT_DEBIT_SEPA" in novalnetDetails.paymentType  %}
			{% if instalmentInfo is not empty %}
				...
				{% for info in instalmentInfo.InstalmentDetails %}
					{%set amount = info.amount/100 %}
					{{ loop.index }} {{ info.reference ? info.reference : "-" }} {{ amount ? amount|currency(currencyIsoCode): "-" }}  {{ info.cycleDate ? info.cycleDate|date("Y-m-d"): "-" }}
				{% endfor %}
			{% endif %}
		{% endif %}
