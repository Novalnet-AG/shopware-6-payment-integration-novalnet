...
class PaymentMethodInstaller
{
    ...
    public function alterPaymentTokenTable(): void
    {
        ...
        if (!empty($istokenTableExists['exists_tbl'])) {
            $istokenColumnExists    = $connection->fetchOne('SHOW COLUMNS FROM `novalnet_payment_token` LIKE "subscription"');

            if (!$istokenColumnExists) {
                $connection->exec('ALTER TABLE `novalnet_payment_token` ADD `subscription` INT(1) DEFAULT 0 COMMENT "Subscription Token" AFTER `expiry_date`;');
            }
        }
    }
        ...
        if ($paymentMethods) {
            ...
            foreach ($paymentMethods as $paymentMethod) {
                ...
                if (empty($paymentMethodId)) {
                    ...
                    if (!is_null($salesChannelPaymentRepo)) {
    ...
    private function updateMediaData(): void
    {
        ...
        foreach ($paymentMethods as $paymentMethod) {
            ...
            if (is_null($paymentMethodEntity->getMediaId())) {
                $mediaId = $this->container->get(MediaProvider::class)->getMediaId($paymentMethod->getPaymentCode(), $this->context);
                $this->container->get('payment_method.repository')->update([
                    [
                        'id'       => $paymentMethodId,
                        'mediaId'  => $mediaId,
...
<p>
    ...
    <strong>Selected payment type:</strong> {{ lastTransaction.paymentMethod.name }}<br>
    {{ lastTransaction.paymentMethod.description }}<br>
    <br>
...
Selected payment type: {{ lastTransaction.paymentMethod.name }}
{{ lastTransaction.paymentMethod.description }}
...
<p>
    ...
    <strong>Gewählte Zahlungsart:</strong> {{ lastTransaction.paymentMethod.name }}<br>
    {{ lastTransaction.paymentMethod.description }}<br>
...
Gewählte Zahlungsart: {{ lastTransaction.paymentMethod.name }}
{{ lastTransaction.paymentMethod.description }}
