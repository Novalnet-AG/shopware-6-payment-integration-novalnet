...
class PaymentMethodInstaller
{
        ...
        if ($paymentMethods) {
            ...
            foreach ($paymentMethods as $paymentMethod) {
                ...
                if (empty($paymentMethodId)) {
                    ...
                    if($paymentMethod->getPaymentCode() == 'novalnetideal'){
                        $this->container->get('payment_method.repository')->update([
                            [
                                'id' => $paymentDetails->getId(),
                                'translations' => [
                                    'en-GB' => [
                                        'name' => $paymentMethod->getName('en-GB'),
                                        'description' => $paymentMethod->getDescription('en-GB'),
                                    ],
                                    'de-DE' => [
                                        'name' => $paymentMethod->getName('de-DE'),
                                        'description' => $paymentMethod->getDescription('de-DE'),
                                    ],
                                ],
                            ],
                        ], $context);
    ...
    private function updateMediaData(): void
    {
        ...
        foreach ($paymentMethods as $paymentMethod) {
            ...
            if ((is_null($paymentMethodEntity->getMediaId()) && $mediaProvider != null) || ($paymentMethod->getPaymentCode() == 'novalnetideal')) {
