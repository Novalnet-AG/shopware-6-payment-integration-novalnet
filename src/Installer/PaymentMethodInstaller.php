...
class PaymentMethodInstaller
{
    ...
    /**
     * Return the name of the payment method based on the given locale.
     * If the locale does not exist, it will return the name for the locale 'de-DE'
     *
     * @param string $locale
     *
     * @return string
    ...
    public function getName(string $locale): string
    {
        ...
        return $this->translations['de-DE']['name'];
    }

    /**
     * Return the description of the payment method based on the given locale.
     * If the locale does not exist, it will return the description for the locale 'de-DE'
     *
     * @param string $locale
     *
     * @return string
    ...
    public function getDescription(string $locale): string
    {
        ...
        return $this->translations['de-DE']['description'];
    }
    ...
    private function addPaymentMethods(): void
    {
        ...
        if (empty($paymentMethodId)) {
            ...
            $paymentData = [
                [
                    ...
                    'position' => -1001,
                    'technicalName' => 'novalnet_payment',
            ...
            $this->context->scope(
                ...
                function (Context $context) use ($paymentData): void {
            ...
            $paymentmethods = $paymentRepository->search($paymentCriteria, $this->context)->first();
            $technicalName = $paymentmethods->getTechnicalName();
            if (!empty($paymentmethods) && (empty($technicalName) || $technicalName !== 'novalnet_payment')) {
        ...
        $customFieldExistsId = $this->customFieldsExist('novalnet');

        if (empty($customFieldExistsId) && $this->container->has('custom_field_set.repository')) {
        ...
        $customFieldsPaymentExistsId = $this->customFieldsExist('novalnetPaymentName');

        if (empty($customFieldsPaymentExistsId) && $this->container->has('custom_field_set.repository')) {
    ...
    /**
     * Returns the ID of the payment method with the given handler identifier.
     *
     * @return string|null The ID of the payment method, or null if it does not exist.
     */
    ...
    /**
     * Retrieves the code of the default locale set in the system.
     *
     * @return string|null The code of the default locale, or null if no default locale is set.
     */
    ...
    /**
     * Updates the custom fields of a payment method with the given name.
     *
     * Updates the custom fields of a payment method with the given name to include the payment method name.
     * The custom fields are updated only if they are empty.
     *
     * @param string $paymentcode The name of the payment method.
     ...
     * @return string|null
     */
    private function customFieldsExist(string $customFieldName): ?string
    {
        $criteria = new Criteria();
    ...
    private function updateMediaData(): void
    {
        ...
        if ($paymentMethodEntity->getMediaId() === null) {
    ...
    private function getPaymentMethodEntity(): void
    {
        ...
        if (!empty($paymentLists)) {
            foreach ($paymentLists as $payment) {
                if ($payment->gethandlerIdentifier() != NovalnetPayment::class) {
                    if (!$payment->getId()) {
                        continue;
                    }
                    ...
                    $channels = $this->container->get('sales_channel.repository')->searchIds(new Criteria(), $this->context);
                    $salesChannelRepo = $this->container->get('sales_channel_payment_method.repository');
                    // Remove payment methods from the sales channel
                    foreach ($channels->getIds() as $channel) {
                        $data = [
                            'salesChannelId' => $channel,
                            'paymentMethodId' => $payment->getId(),
                        ];
                        $salesChannelRepo->delete([$data], $this->context);
                    }
                }
            }
        }
    ...
    {
        $criteria = new Criteria();
        $criteria->addFilter(new EqualsFilter('handlerIdentifier', NovalnetPayment::class));
    ...
    public function alterPaymentTokenTable(): void
    {
        ...
        $isTableExists = $connection->executeQuery('
			...
			AND table_schema = database()')->fetchAllAssociative();
        if (!empty($isTableExists[0]['exists_tbl'])) {
    ...
    private function getHtmlTemplateEn(): string
    {
		...
		<p>
			...
			<strong>Selected payment type:</strong> {% if paymentName is not empty %} {{ paymentName }} {% else %} {{ order.transactions|last.paymentMethod.translated.name }} {% endif %} <br>
			{% if paymentDescription is not empty %} {{ paymentDescription }} {% else %} {{ order.transactions|last.paymentMethod.translated.description }} {% endif %} <br>
			<br>
					...
					<table width="40%" style="font-family:Arial, Helvetica, sans-serif; border: 1px solid;border-color: #bcc1c7;text-align: center;font-size:12px;">
						...
						<tbody>
							...
							{% for value in instalmentData %}
								...
								<tr>
									<td style="border-bottom:1px solid #cccccc;">{{ count }}</td>
									<td style="border-bottom:1px solid #cccccc;">{{ instalmentData[count].reference ? instalmentData[count].reference : "-" }}</td>
									<td style="border-bottom:1px solid #cccccc;">{{ amount ? amount|currency(currencyIsoCode): "-" }}</td>
									<td style="border-bottom:1px solid #cccccc;">{{ instalmentData[count].cycleDate ? instalmentData[count].cycleDate|date("Y-m-d"): "-" }}</td>
								<tr>
								{% set count = count + 1 %}
							{% endfor %}
						</tbody>
    ...
    private function getPlainTemplateEn(): string
    {
		...
		{% if paymentDescription is not empty %} {{ paymentDescription }} {% else %} {{ order.transactions|last.paymentMethod.translated.description }} {% endif %}
		...
		{% if "INSTALMENT_INVOICE" in novalnetDetails.paymentType or "INSTALMENT_DIRECT_DEBIT_SEPA" in novalnetDetails.paymentType  %}
			{% if instalmentInfo is not empty %}
				...
				{% for value in instalmentData %}
					{%set amount = value.amount/100 %}
					{{ count }} {{ instalmentData[count].reference ? instalmentData[count].reference : "-" }} {{ amount ? amount|currency(currencyIsoCode): "-" }}  {{ instalmentData[count].cycleDate ? instalmentData[count].cycleDate|date("Y-m-d"): "-" }}
					{% set count = count + 1 %}
				{% endfor %}
			{% endif %}
		{% endif %}
    ...
    private function getHtmlTemplateDe(): string
    {
        return '<div style="font-family:Arial, Helvetica, sans-serif; font-size:12px;">
			...
			<p>
				...
				{% endif %}
				<br>
				<strong>Gewählte Zahlungsart:</strong> {% if paymentName is not empty %} {{ paymentName }} {% else %} {{ order.transactions|last.paymentMethod.translated.name }} {% endif %} <br>
				{% if paymentDescription is not empty %} {{ paymentDescription }} {% else %} {{ order.transactions|last.paymentMethod.translated.description }} {% endif %} <br>
				<br>
     ...
     * The template is identified by its technical name 'novalnet_order_confirmation_mail'.
     *
