...
export default class NovalnetGooglePayPayment extends Plugin {
	...
	showButton(paymentName, hideDefault) {
		if (hideDefault) {
			...
			var cancelButton = 'line-height: 2.5rem; margin-top: -25px;';
		} else {
			var displayType = 'none';
			var changePaymentForm = 'inline';
			var cancelButton = 'line-height: 2.125rem; margin-top: 0px;';
		}
		...
		if (document.querySelector('#novalnetchangePaymentForm button[class="btn btn-outline-secondary"]') != undefined && document.querySelector('#novalnetchangePaymentForm button[class="btn btn-outline-secondary"]') != null) {
			document.querySelector('#novalnetchangePaymentForm button[class="btn btn-outline-secondary"]').style.cssText = cancelButton;
		}
		...
		if (document.getElementById(paymentName + "ZeroAmountNotify") != undefined && document.getElementById(paymentName + "ZeroAmountNotify") != null) {
				...
				callbacks: {
						...
						if (response.result && response.result.status) {
							...
							if (response.result.status == 'SUCCESS') {
								...
								if (DomAccess.getDataAttribute(element, 'data-page-type') == 'checkoutPage') {
									...
									if (element != undefined && element.className == 'nn-apple-pay-button') {
										...
										if (response.transaction.doRedirect && response.transaction.doRedirect != false) {
											...
											client.post(successUrl, JSON.stringify(payLoad), (res) => {
												...
												} else if (novalnetResponse.success == false) {
													bookingResult({ status: "FAILURE", statusText: novalnetResponse.message });
												} else {
									...
									if (element != undefined && element.className == 'nn-apple-pay-changePayment') {
					...
					onPaymentButtonClicked: function (clickResult) {
						...
						if (tosInput != undefined && !tosInput.checked && DomAccess.getDataAttribute(element, 'data-page-type') == 'checkoutPage') {
							...
							tosInput.classList.add('is-invalid');
							tosInput.focus();
							return false;
	...
	_displayErrorMsgSubs(errorMessage) {
		const elements = document.getElementsByClassName('alert alert-danger d-flex align-items-center');
		...
		parentDiv.className = "alert alert-danger d-flex align-items-center"; childDiv1.className = "alert-content-container"; childDiv2.className = "alert-content"; spanTag.className = "icon icon-blocked";
