...
export default class NovalnetGooglePayPayment extends Plugin {
	...
	init() {
		...
		if (selectedPaymentId !== undefined && selectedPaymentId !== null && googlePayPaymentId !== undefined && googlePayPaymentId.value === selectedPaymentId.value)
		{
			this.showButton('novalnetgooglepay', true);
		}
		
		if (selectedPaymentId !== undefined && selectedPaymentId !== null && applePayPaymentId !== undefined && applePayPaymentId.value === selectedPaymentId.value)
		{
			...
			paymentRadioButton.forEach((payment) => {
				payment.addEventListener('click', () => {
					...
					if (selectedPaymentId !== undefined && selectedPaymentId !== null && googlePayPaymentId !== undefined && googlePayPaymentId.value === selectedPaymentId.value)
					{
						...
						this.showButton('novalnetgooglepay', true);
					} else if (selectedPaymentId !== undefined && selectedPaymentId !== null && applePayPaymentId !== undefined && applePayPaymentId.value === selectedPaymentId.value) 
					{
	...
	showButton (paymentName, hideDefault) {
		if (hideDefault)
		{
			...
			document.querySelector('#novalnetchangePaymentForm button[class="btn btn-primary"]').style.display = "none";
		} else {
			document.getElementById(paymentName + "-payment").style.display = "none";
			document.querySelector('#novalnetchangePaymentForm button[class="btn btn-primary"]').style.display = "inline";
		}
	}
