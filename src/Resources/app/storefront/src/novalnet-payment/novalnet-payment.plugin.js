...
export default class NovalnetPayment extends Plugin {
    ...
    init() {
		...
		paymentName.forEach((element) => {
			...
			if( selectedPaymentId !== undefined && selectedPaymentId !== null && paymentMethodId !== undefined && paymentMethodId !== null && selectedPaymentId.value === paymentMethodId.value )
			{
			...
			} else if (SepaPaymentTypes.includes(element.value) || InvoicePaymentTypes.includes(element.value))
			{
                ...
                const mandateInfo = document.getElementById(element.value + 'Mandate');
                ...
                const AccountHolder = document.getElementById('novalnetdirectdebitachAccountHolder');
                this.handleInputKeyPress(AccountHolder);
            }
	...
	validatePaymentForm(paymentName, button, buttonType, event)
	{
		...
		if (paymentName == 'novalnetcreditcard' || ['novalnetsepa', 'novalnetsepaguarantee', 'novalnetsepainstalment'].includes(paymentName) || paymentName == 'novalnetdirectdebitach' || paymentName == 'novalnetmbway')
		{	  
			if (paymentMethodId !== undefined && paymentMethodId.value !== '' && selectedPaymentId.value === paymentMethodId.value)
			{
				...
				} else if (['novalnetsepa', 'novalnetsepaguarantee', 'novalnetsepainstalment'].includes(paymentName))
				{
					...
					if ((radioInputChecked === undefined || radioInputChecked === null || radioInputChecked.value == 'new') && (iban === undefined || (iban !== undefined && iban.value === '')))
					{
