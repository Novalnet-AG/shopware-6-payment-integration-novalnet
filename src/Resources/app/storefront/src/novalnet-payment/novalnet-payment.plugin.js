...
export default class NovalnetPayment extends Plugin {
    init() {
		...
		if( novalnetpayId != null) {
            ...
            if (subscriptionSubmitButton != undefined) {
            ...
            paymentForm.validationResponse((data) => {
				...
				if(data.result.statusCode == '340' && data.result.nnpf_subType == 'NO_PAYMENT_METHODS'){
					var paymentMethodElement = document.querySelector('#paymentMethod' + novalnetpayId.value);
					if (paymentMethodElement) {
						if (paymentMethodElement.checked) {
							...
							me._displayErrorMsg(document.getElementById('iframeErrorMessage').value, 'info');
							return;
						}
					}
            ...
            paymentForm.walletResponse({
                onProcessCompletion: function (response) {
                    if (response.result.status == 'SUCCESS') {
						if (document.querySelector("#isSubscriptionRestrict") != undefined ) {
							var subscriptionRestrict = document.querySelector("#isSubscriptionRestrict").value;
							if(subscriptionRestrict == 1) {
								window.scrollTo(0, 0);
                ...
                onPaymentButtonClicked: function(response) {
					if (document.querySelector("#isSubscriptionRestrict") != undefined ) {
							var subscriptionRestrict = document.querySelector("#isSubscriptionRestrict").value;
							if(subscriptionRestrict == 1){
								window.scrollTo(0, 0);
							...
							paymentForm.getPayment((function(paymentDetails) {
								if (paymentDetails.result.statusCode == '100' || paymentDetails.result.status == 'SUCCESS') {
									...
									document.querySelector('#confirmOrderForm').submit();
									 ...
									 me._displayErrorMsg(paymentDetails.result.message, 'danger');
									 me._showSubmitForm();
								}
							}));
						}
    ...
    _displayErrorMsg(errorMessage, messageType) {
        ...
        if(messageType == 'danger'){
			parentDiv.className= "alert alert-danger d-flex align-items-center";
		} else {
			parentDiv.className= "alert alert-info d-flex align-items-center";
		}
        childDiv1.className= "alert-content-container";childDiv2.className= "alert-content";spanTag.className= "icon icon-blocked";
