    ...
    methods: {
        ...
        createdComponent() {
			...
			if(this.actualConfigData != undefined && this.isRequested != this.selectedSalesChannelId)
			{
				...
				this.apiActivationKey	= this.actualConfigData['NovalnetPayment.settings.clientId'];
				this.paymentAccessKey	= this.actualConfigData['NovalnetPayment.settings.accessKey'];
        ...
        configureWebhookUrl() {
			...
			if (productActivationKey == '' || accessKey == '')
			{
				this.createNotificationError({
							title: this.$tc('novalnet-payment.settingForm.titleError'),
							message: this.$tc('novalnet-payment.settingForm.failureMessage')
			...
			if( this.NovalnetPaymentCallBackUrl)
			{
				...
				if(window.confirm(this.$tc('novalnet-payment.settingForm.webhookUrlConfirm'))){
