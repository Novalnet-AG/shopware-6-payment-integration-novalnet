...
class PaymentEventSubscriber implements EventSubscriberInterface
{
    ...
    {
        if (!is_null($this->request) && !empty($this->request->request->get('paymentMethodId'))) {
            $formDataSession = [];
            $formData     = [];
            $paymentMethod   = '';
            ...
            if (! empty($paymentMethod)) {
                if ($this->request->request->get('doForceSepaPayment')) {
                    ...
                    $parameters = (array) $this->request->request;
                    foreach ($parameters as $parameter) {
                        if(isset($parameter[$paymentMethod . 'FormData'])) {
                            $formData = $parameter[$paymentMethod . 'FormData'];
                        }
                    }
                }
