...
class StorefrontRenderEventSubscriber implements EventSubscriberInterface
{
    ...
    public function onStorefrontRender(StorefrontRenderEvent $event): void
    {
        ...
        if (!empty($_SERVER['REQUEST_URI']) && !preg_match('/checkout\/confirm/', (string) $_SERVER['REQUEST_URI'])) {
        ...
        if (!empty($_SERVER['REQUEST_URI']) && preg_match('/account\/payment/', (string) $_SERVER['REQUEST_URI'])) {
            $this->helper->setSession('novalnetAccountPaymentPage', true);
        } elseif ($this->helper->hasSession('novalnetAccountPaymentPage')) {
            $this->helper->removeSession('novalnetAccountPaymentPage');
        }
    }
}
