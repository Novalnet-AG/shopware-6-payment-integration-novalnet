...
class FrontendController extends StorefrontController
{
    ...
    #[Route(path: '/novalnet/googlepay', name: 'frontend.novalnet.googlePayRedirect', methods: ['POST'], defaults: ['csrf_protected' => false], options: ['seo' => false])]
    ...
    public function storeCustomerData(Request $request, SalesChannelContext $salesChannelContext): JsonResponse
    {
        ...
        foreach ([
            'pan_hash', 'unique_id', 'iban', 'bic', 'wallet_token', 'token'
        ] as $key) {
            if (! empty($data[$key])) {
    ...
    #[Route(path: '/novalnet/returnAction', name: 'frontend.novalnet.returnAction', methods: ['POST'], defaults: ['csrf_protected' => false], options: ['seo' => false])]
    ...
    public function formTransactionData(array $response): array
    {
        $insertData = [
            ...
            'paymentType' => $response['transaction']['payment_type'] != 'DIRECT_DEBIT_SEPA' ? 'novalnet' . strtolower($response['transaction']['payment_type']) : 'novalnetsepa'
        ];
