...
class FrontendController extends StorefrontController
{
    ...
    /**
     * @Route("/store/changePaymentData", name="frontend.novalnet.storeCustomerData", options={"seo"="false"}, defaults={"XmlHttpRequest"=true}, methods={"GET", "POST"})
     */
    public function storeCustomerData(Request $request, SalesChannelContext $context): JsonResponse
    {
        ...
        if ((isset($data['booking_details']['do_redirect']) && ($data['booking_details']['do_redirect'] == 1)) || (isset($data['payment_details']['process_mode']) && $data['payment_details']['process_mode'] == 'redirect')) {
            $parameters['transaction']['return_url'] = $parameters['transaction']['error_return_url'] = $this->generateAbsoluteUrl();
        }
   ...
   /**
     ...
     * @return string
     */
    public function generateAbsoluteUrl() : string
    {
		$requested = $this->helper->getCurrentRequest();
        ...
        if(!empty($host)) {
            return   $host.'/novalnet/returnAction';

        } else {
		return $this->router->generate('frontend.novalnet.returnAction', [], UrlGeneratorInterface::ABSOLUTE_URL);
        }
    }
    
    /**
     * @Route("/novalnet/returnAction", name="frontend.novalnet.returnAction", options={"seo"="false"}, methods={"GET", "POST"})
     */
        ...
        $paymentType = $response['transaction']['payment_type'] ?? 'NOVALNET_PAYMENT';
            ...
            $insertData = [
                ...
                'customerNo' => !empty($response['customer']['customer_no']) ? $response['customer']['customer_no'] : '',
                 'additionalDetails' => [
                    'payment_name' => $this->helper->getUpdatedPaymentName($paymentType, $this->helper->getLocaleCodeFromContext($context->getContext(), true)),
                    'change_payment' => true,
                    'subscription' => $data
                ]
            ];
