...
class FrontendController extends StorefrontController
{
    ...
    public function successOrder(Request $request, SalesChannelContext $salesChannelContext)
    {
        ...
        try {
            ...
            if (!empty($serverData['orderId'])) {
                ...
                $isSubscription = $this->helper->isSubscriptionOrder($orderId, $salesChannelContext->getContext());
				...
				if($isSubscription == null){
					...
					$data = new RequestDataBag([$serverData['paymentName'].'FormData' => ['walletToken' => $requestData['transaction']['token'], 'doRedirect' => $requestData['transaction']['doRedirect']], 'ExpressCheckout' => true, 'isSubscriptionOrder' => true, 'changePayment' => true, 'subParentOrderNumber' => $isSubscription]);
				}
            } else {
    ...
    /**
     * @Route("/novalnet/couponCodeUpdate", name="frontend.novalnet.couponCodeUpdate", options={"seo"="false"}, defaults={"csrf_protected"=false, "XmlHttpRequest"=true}, methods={"GET", "POST"})
     */
