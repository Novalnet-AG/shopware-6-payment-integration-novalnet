...
class FrontendController extends StorefrontController
{
    ...
    */
    ...
    public function addToCart(Cart $cart, Request $request, SalesChannelContext $salesChannelContext): JsonResponse
    {
        $data = $this->helper->unserializeData((string) $request->getContent());
        try {
    ...
    public function loadShippingMethod(Request $request, SalesChannelContext $salesChannelContext): JsonResponse
    {
        ...
        if (!empty($data)) {
            ...
            if (!empty($shippingMethods)) {
                $salesChannelContext = $this->cartHelper->updateShippingMethod($shippingMethods[0]['identifier'], $salesChannelContext);
            }
            $recalculatedCart    = $this->cartHelper->getCalculatedCart($salesChannelContext);
            $orderDetails        = $this->cartHelper->getFormattedCart($recalculatedCart, $salesChannelContext);
    ...
    public function successOrder(Request $request, SalesChannelContext $salesChannelContext)
    {
        ...
        $previousShippingId = $salesChannelContext->getShippingMethod()->getId();
        ...
        try {
            if (is_null($salesChannelContext->getCustomer())) {
                ...
                $salesChannelContext = $this->cartHelper->getSalesChannelContext($salesChannelContext->getSalesChannel()->getId(), $newToken);
            ...
            $options = [
                ...
                SalesChannelContextService::DOMAIN_ID => $request->attributes->get(SalesChannelRequest::ATTRIBUTE_DOMAIN_ID, ''),
                SalesChannelContextService::SHIPPING_METHOD_ID => $previousShippingId,
                SalesChannelContextService::CURRENCY_ID => $previousCurrencyId ?? $salesChannelContext->getCurrency()->getId(),
                SalesChannelContextService::BILLING_ADDRESS_ID => $salesChannelContext->getCustomer()->getDefaultBillingAddress()->getId(),
                SalesChannelContextService::SHIPPING_ADDRESS_ID => $salesChannelContext->getCustomer()->getDefaultShippingAddress()->getId(),
            ...
            if (!empty($serverData['paymentMethodId'])) {
                $options[SalesChannelContextService::PAYMENT_METHOD_ID] = $serverData['paymentMethodId'];
            }
            ...
            if (!empty($serverData['orderId'])) {
                ...
                $order = $this->helper->getOrderCriteria($salesChannelContext->getContext(),$orderId);
                ...
                $datas = new RequestDataBag([$serverData['paymentName'] . 'FormData' => ['walletToken' => $requestData['transaction']['token'], 'doRedirect' => $requestData['transaction']['doRedirect']], 'ExpressCheckout' => true]);
                $data = new Request([$serverData['paymentName'] . 'FormData' => ['walletToken' => $requestData['transaction']['token'], 'doRedirect' => $requestData['transaction']['doRedirect']], 'ExpressCheckout' => true]);
                $data->attributes->set(SalesChannelRequest::ATTRIBUTE_DOMAIN_ID, $request->attributes->get(SalesChannelRequest::ATTRIBUTE_DOMAIN_ID, ''));
            } else {
                ...
                if ($cart->hasExtension('novalnetConfiguration')) {
                    $novalnetConfiguration = $cart->getExtension('novalnetConfiguration')->getVars();
                ...
                $datas = new RequestDataBag(['tos' => true, 'revocation' => true, 'isSubscriptionOrder' =>  $isSubscriptionOrder, $serverData['paymentName'] . 'FormData' => ['walletToken' => $requestData['transaction']['token'], 'doRedirect' => !empty($requestData['transaction']['doRedirect']) ? $requestData['transaction']['doRedirect'] : 0 ], 'ExpressCheckout' => true]);
                $data = new Request(['tos' => true, 'revocation' => true, 'isSubscriptionOrder' =>  $isSubscriptionOrder, $serverData['paymentName'] . 'FormData' => ['walletToken' => $requestData['transaction']['token'], 'doRedirect' => !empty($requestData['transaction']['doRedirect']) ? $requestData['transaction']['doRedirect'] : 0], 'ExpressCheckout' => true]);
                $data->attributes->set(SalesChannelRequest::ATTRIBUTE_DOMAIN_ID, $request->attributes->get(SalesChannelRequest::ATTRIBUTE_DOMAIN_ID, ''));
                $this->addAffiliateTracking($datas, $request->getSession());
                $orderId = $this->orderService->createOrder($datas, $salesChannelContext);
            }
        } catch (ConstraintViolationException $formViolations) {
            return new JsonResponse(['success' => true, 'message' => $formViolations]);
        ...
        try {
            ...
            $response = $this->paymentProcessor->pay($orderId, $data, $salesChannelContext, $finishUrl, $errorUrl);
            ...
            return  new JsonResponse(['success' => true, 'url' => $finishUrl]);
        } catch (\Exception $e) {
    ...
    public function authenticateGooglePay(Request $request, SalesChannelContext $salesChannelContext)
    {
        ...
        $returnURL = $this->generateUrl('frontend.checkout.finish.page', ['orderId' => $orderId]);

        $paymentTransaction = new PaymentTransactionStruct($transaction->getId(), $returnURL);
        $transactionDetails = $this->transactionHelper->getOrderTransactionDetails($transaction->getId(), $salesChannelContext->getContext());
     ...
     * @return void
     */
    private function addAffiliateTracking(RequestDatabag $request, SessionInterface $session): void
    {
        ...
        if ($affiliateCode) {
            $request->set(AffiliateTrackingListener::AFFILIATE_CODE_KEY, $affiliateCode);
        }
    ...
    public function couponCodeUpdate(Cart $cart, Request $request, SalesChannelContext $salesChannelContext): JsonResponse
    {
        $data = $this->helper->unserializeData((string) $request->getContent());
