...
class WebhookController extends StorefrontController
{
     ...
     * {@inheritdoc}
     */
    public function __construct(private readonly NovalnetHelper $helper, private readonly NovalnetOrderTransactionHelper $transactionHelper, private readonly TranslatorInterface $translator, private readonly OrderTransactionStateHandler $orderTransactionStateHandler, private readonly StateMachineRegistry $stateMachineRegistry, private readonly EntityRepository $orderTransactionRepository, private readonly AbstractMailService $mailService)
    {
    }
    ...
    public function statusAction(Request $request, SalesChannelContext $salesChannelContext): Response
    {
        ...
        if (!empty($this->eventData['transaction']['order_no'])) {
            ...
            $this->salesChannelId = $order !== null ? $order->getSalesChannel()->getId() : $this->salesChannelContext->getSalesChannel()->getId();
        } else {
            $this->salesChannelId = $this->salesChannelContext->getSalesChannel()->getId();
        }
        ...
        $this->eventData['transaction']['currency'] ??= $this->orderReference->getCurrency();
    ...
    public function authenticateRequestIp(): bool
    {
        ...
        if (!empty($this->novalnetHostName)) {
            ...
            $deactivateIp = $this->helper->getNovalnetPaymentSettings('NovalnetPayment.settings.deactivateIp', $this->salesChannelId);
            if (empty($requestReceivedIp) && empty($deactivateIp)) {
                $this->response = ['message' => 'Unauthorised access from the IP :' . $this->helper->getIp()];
            ...
            if (empty($this->eventData[$category])) {
                ...
                foreach ($parameters as $parameter) {
                    if (!isset($this->eventData[$category][$parameter])) {
                        ...
                        return false;
                    } elseif (in_array($parameter, ['tid', 'parent_tid']) && !preg_match('/^\d{17}$/', (string) $this->eventData[$category][$parameter])) {
    ...
    public function validateChecksum(): bool
    {
        ...
        $paymentAccessKey = $this->helper->getNovalnetPaymentSettings('NovalnetPayment.settings.accessKey', $this->salesChannelId);
        if (!empty($paymentAccessKey)) {
            $tokenString .= strrev((string) $paymentAccessKey);
        }
    ...
    public function getOrderReference(): bool
    {
        ...
        if (!empty($this->eventData['transaction']['order_no']) || !empty($this->parentTid)) {
            ...
            $paymentAccessKey = $this->helper->getNovalnetPaymentSettings('NovalnetPayment.settings.accessKey', $this->salesChannelId);
            $completeStatus = $this->helper->getNovalnetPaymentSettings('NovalnetPayment.settings.completeStatus', $this->salesChannelId);
            ...
            if ($this->orderReference === null) {
                if ($this->helper->isSuccessStatus($this->eventData)) {
                    if ($this->eventData['transaction']['payment_type'] === 'ONLINE_TRANSFER_CREDIT' && !empty($this->parentTid)) {
                        $response = $this->helper->sendPostRequest(['transaction' => ['tid' => $this->parentTid]], $this->helper->getActionEndpoint('transaction_details'), $paymentAccessKey);
                    ...
                    if (empty($customFields['novalnet_comments']) || !preg_match('/' . $this->eventData['transaction']['tid'] . '/', (string) $customFields['novalnet_comments'])) {
                        $this->updateInitialPayment($paymentMethod);
                    } else {
                        $this->response = ['message' => 'Novalnet Callback executed. The Transaction ID already existed'];
                    }
    ...
    public function updateInitialPayment(PaymentMethodEntity $paymentMethod): void
    {
        ...
        $orderTransactionDetails = $this->transactionHelper->getOrderTransactionDetails($this->orderTransaction->getId(), $this->context);
        if (method_exists($paymentMethodInstance, 'checkTransactionStatus')) {
            ...
            $paymentMethodInstance->checkTransactionStatus($orderTransactionDetails, $this->eventData, $this->salesChannelContext, '1');
    ...
    {
        $mailTo = $this->helper->getNovalnetPaymentSettings('NovalnetPayment.settings.mailTo', $this->salesChannelId);

        if (!empty($mailTo) && !empty($this->response['message'])) {
            $toEmail = explode(',', (string) $mailTo);
            $data = new DataBag();
    ...
    private function creditProcess(): string
    {
        ...
        if ((int) $this->orderReference->getPaidAmount() < (int) $this->orderReference->getAmount() && in_array($this->eventData['transaction']['payment_type'], ['INVOICE_CREDIT', 'CASHPAYMENT_CREDIT', 'MULTIBANCO_CREDIT', 'ONLINE_TRANSFER_CREDIT', 'DEBT_COLLECTION_DE'])) {
            ...
            if ($paidAmount >= (int) $this->orderReference->getAmount()) {
                try {
        ...
        } elseif (!in_array($this->eventData['transaction']['payment_type'], ['INVOICE_CREDIT', 'CASHPAYMENT_CREDIT', 'MULTIBANCO_CREDIT'])) {
            $paidAmount = (int) $this->eventData['transaction']['amount'];
            $callbackComments = $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.creditMessage', [], null, $this->locale), $this->parentTid, $this->formattedAmount, date('d/m/Y H:i:s'), $this->eventTid);
        } else {
            return 'Novalnet callback received. The transaction amount is already paid.';
        }
        ...
        if ($paidAmount >= (int) $this->orderReference->getAmount()) {
            try {
                ...
                if (!in_array($this->eventData['transaction']['payment_type'], ['INVOICE_CREDIT', 'CASHPAYMENT_CREDIT', 'MULTIBANCO_CREDIT'])) {
                    $this->transactionHelper->subscriptionCreditStatusUpdate($this->order->getId(), $this->context, $this->locale, $this->salesChannelId, $this->order->getOrderCustomer()->getCustomerId());
                }
            } catch (IllegalTransitionException) {
            }
        } else {
            $this->orderTransactionStateHandler->paidPartially($this->orderTransaction->getId(), $this->context);
        }
    ...
    private function transactionCaptureVoid(): string
    {
        ...
        if (in_array($this->orderReference->getGatewayStatus(), ['ON_HOLD', 'PENDING'])) {
            $paymentType = $this->helper->getUpdatedPaymentType($this->orderReference->getPaymentType());

            if ($this->eventType === 'TRANSACTION_CAPTURE' && $paymentType === 'INVOICE') {
            ...
            $upsertData = [
                'id' => $this->orderReference->getId(),
                'gatewayStatus' => $this->eventData['transaction']['status'],
                'amount' => !empty($this->eventData['transaction']['amount']) ? $this->eventData['transaction']['amount'] : $this->orderReference->getAmount(),
            ];

            if ($this->eventType === 'TRANSACTION_CAPTURE') {
                if (in_array($this->eventData['transaction']['status'], ['CONFIRMED', 'PENDING'])) {
                    if (!empty($this->orderReference->getAdditionalDetails()) && in_array($paymentType, ['INVOICE', 'GUARANTEED_INVOICE', 'PREPAYMENT', 'INSTALMENT_INVOICE'])) {
                        $appendComments = false;
                        $additionalDetails = $this->helper->unserializeData($this->orderReference->getAdditionalDetails());
                        if (!empty($additionalDetails['bankDetails'])) {
                            $this->eventData['transaction']['bank_details'] = $additionalDetails['bankDetails'];
                        }

                        $callbackComments .= $this->helper->formBankDetails($this->eventData, $this->context, $this->order->getLanguageId()) . $this->newLine;
                    }
                    ...
                    if ($this->eventData['transaction']['status'] === 'CONFIRMED') {
                        ...
                        if (in_array($paymentType, ['INSTALMENT_INVOICE', 'INSTALMENT_DIRECT_DEBIT_SEPA'])) {
                            $upsertData['additionalDetails'] = $this->helper->unserializeData($this->orderReference->getAdditionalDetails());
                    ...
                    if (in_array($paymentType, ['INVOICE', 'GUARANTEED_INVOICE', 'INSTALMENT_INVOICE', 'INSTALMENT_DIRECT_DEBIT_SEPA'])) {
                        $this->transactionHelper->prepareMailContent($this->order, $this->salesChannelContext, $callbackComments);
                    }
                }
                $message = 'NovalnetPayment.text.confirmMessage';
            } else {
				$appendComments = false;
				$callbackComments .= $this->helper->formBankDetails($this->eventData, $this->context, $this->order->getLanguageId()) . $this->newLine;
                $message = 'NovalnetPayment.text.faliureMessage';
            }

            $callbackComments .= $this->newLine . sprintf($this->translator->trans($message, [], null, $this->locale), date('d/m/Y H:i:s'));
            $this->transactionHelper->postProcess($this->orderTransaction, $this->context, $callbackComments, $upsertData, $appendComments);

            if (in_array($paymentType, ['INVOICE', 'GUARANTEED_INVOICE', 'INSTALMENT_INVOICE', 'INSTALMENT_DIRECT_DEBIT_SEPA']) && in_array($this->eventData['transaction']['status'], ['CONFIRMED', 'PENDING'])) {
                $this->transactionHelper->prepareMailContent($this->order, $this->salesChannelContext, $callbackComments);
            }
            ...
            try {
                if ($this->eventData['transaction']['status'] === 'CONFIRMED') {
                    ...
                    $this->transactionHelper->managePaymentStatus('process', $this->orderTransaction->getId(), $this->context);
                } elseif ($this->eventType === 'TRANSACTION_CANCEL') {
                    $this->orderTransactionStateHandler->cancel($this->orderTransaction->getId(), $this->context);
                }
            } catch (IllegalTransitionException) {
            }
        } else {
            $this->response['message'] = 'Order already captured';
        }
    ...
    private function instalmentProcess(): string
    {
        ...
        if (!empty($instalmentData['cycle_amount']) && !empty($instalmentData['cycles_executed']) && isset($instalmentData['pending_cycles'])) {
            ...
            $comments .= $this->newLine . $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.instalmentPrepaidMessage', [], null, $this->locale), $this->parentTid, $this->eventTid, $this->formattedAmount, date('Y-m-d')) . $this->newLine;
            $upsertData['id'] = $this->orderReference->getId();
    ...
    private function updateInstalmentInfo(): string
    {
        ...
        if (!empty($this->eventData['transaction']['bank_details'])) {
            $configurationDetails['bankDetails'] = $this->eventData['transaction']['bank_details'];
        }
        $instalmentData = $this->eventData['instalment'];
        $configurationDetails['InstalmentDetails'][$instalmentData['cycles_executed']] = [
            'amount' => $instalmentData['cycle_amount'],
            'cycleDate' => !empty($instalmentData['next_cycle_date']) ? date('Y-m-d', strtotime((string) $instalmentData['next_cycle_date'])) : '',
    ...
    private function chargebackProcess(): string
    {
        ...
        if (!empty($this->eventData['transaction']['amount'])) {
            if ($this->orderReference->getGatewayStatus() === 'CONFIRMED') {
                ...
                $this->transactionHelper->subscriptionStatusUpdate($this->order->getId(), $this->context, $this->locale);
            }
        } else {
    ...
    private function transactionUpdate(): string
    {
        ...
        if (in_array($transactionStatus, ['CONFIRMED', 'PENDING', 'ON_HOLD', 'DEACTIVATED'])) {
            if (in_array($this->eventData['transaction']['update_type'], ['DUE_DATE', 'AMOUNT_DUE_DATE'])) {
                ...
                $dueDate = date('d/m/Y', strtotime((string) $this->eventData['transaction']['due_date']));
        ...
        try {
            $completeStatus = $this->helper->getNovalnetPaymentSettings('NovalnetPayment.settings.completeStatus', $this->salesChannelId);
            $onHoldStatus = $this->helper->getNovalnetPaymentSettings('NovalnetPayment.settings.onHoldStatus', $this->salesChannelId);

            if ($transactionStatus === 'DEACTIVATED') {
                $this->orderTransactionStateHandler->cancel($orderTransactionId, $salesChannelContext);
            } elseif ($transactionStatus === 'CONFIRMED' && $this->eventData['transaction']['amount'] === 0 && $paymentType !== 'PREPAYMENT') {
    ...
    private function transactionrefund(): string
    {
        ...
        if (!empty($refundAmount) && $this->orderReference->getAmount() > $this->orderReference->getRefundedAmount()) {
            ...
            } elseif ($this->orderReference->getGatewayStatus() !== 'PENDING' && !preg_match('/INSTALMENT/', $this->orderReference->getpaymentType())) {
                $this->orderTransactionStateHandler->refundPartially($this->orderTransaction->getId(), $this->salesChannelContext->getContext());
            }
        } else {
            $callbackComments = 'The order is already fully refunded';
        }
    ...
    private function instalmentCancelProcess(): string
    {
        ...
        if (!empty($this->eventData['instalment']['cancel_type'])) {
            if (isset($this->eventData['transaction']['refund'])) {
                $callbackComments .= $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.instalmentRefundComment', [], null, $this->locale), $this->parentTid, date('Y-m-d H:i:s'));
