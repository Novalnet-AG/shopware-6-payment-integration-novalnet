    ...
    *   @return Response
    ...
    public function getOrderReference() :bool
    {
        ...
        if (! empty($this->eventData ['transaction'] ['order_no']) || ! empty($this->parentTid)) {
            ...
            if (!empty($this->orderTransaction)) {
                $paymentMethod = $this->transactionHelper->getPaymentMethodById($this->orderTransaction->getPaymentMethodId(), $this->context);
                if (!empty($paymentMethod)) {
                    $this->paymentMethodName = $this->helper->getPaymentMethodName($paymentMethod);
                }
            }
            
            if (!$this->helper->checkString($this->paymentMethodName) && empty($this->eventData ['custom']['change_payment'])) {
                $this->response = ['message' => 'Order Reference not exist in Database!'];
                return false;
            }
            ...
            if (isset($this->eventData ['custom']['change_payment']) && !empty($this->eventData ['custom']['change_payment'])) {
                if ($this->helper->isSuccessStatus($this->eventData)) {
                    ...
                    if (empty($orderData)) {
                        $this->updateChangePayment($this->eventData);
    ...
    private function transactionCaptureVoid(): string
    {
        ...
        if (in_array($this->orderReference->getGatewayStatus(), ['ON_HOLD', 'PENDING'])) {
            ...
            $callbackComments .= $this->newLine . sprintf($this->translator->trans((string)$transactionMsg, [], null, $this->locale), date('d/m/Y H:i:s')) . $this->newLine;
    ...
    public function updateSubscriptionData()
    {
        ...
        if ($schemaManager->tablesExist(array('novalnet_subscription')) == true) {
            $data = $connection->fetchOne(sprintf("SELECT `id` FROM `novalnet_subscription` WHERE `order_id` = UNHEX('%s')", $this->order->getId()));
            if (!empty($data)) {
                if (in_array($this->eventData['transaction']['status'], ['ON_HOLD',  'PENDING',  'CONFIRMED'])) {
                    $connection->exec(sprintf("UPDATE `novalnet_subscription` SET `status` = 'ACTIVE', payment_method_id = UNHEX('%s') WHERE `order_id` = UNHEX('%s')", $this->orderTransaction->getPaymentMethodId(), $this->order->getId()));
                    $connection->exec(sprintf("UPDATE `novalnet_subs_cycle` SET `status` = 'SUCCESS', payment_method_id = UNHEX('%s') WHERE `order_id` = UNHEX('%s')", $this->orderTransaction->getPaymentMethodId(), $this->order->getId()));
                } else {
                    $connection->exec(sprintf("UPDATE `novalnet_subscription` SET `status` = 'CANCELLED', cancel_reason = 'Parent order getting failed', cancelled_at = '%s', payment_method_id = UNHEX('%s') WHERE `order_id` = UNHEX('%s')", date('Y-m-d H:i:s'), $this->orderTransaction->getPaymentMethodId(), $this->order->getId()));
                    $connection->exec(sprintf("UPDATE `novalnet_subs_cycle` SET `status` = 'FAILURE', payment_method_id = UNHEX('%s') WHERE `order_id` = UNHEX('%s')", $this->orderTransaction->getPaymentMethodId(), $this->order->getId()));
                }
            }
        }
    }
    
    public function updateChangePayment(array $input)
    {
        ...
        $insertData = [
            ...
            'customerNo' => !empty($input['customer']['customer_no']) ? $this->eventData['customer']['customer_no'] : '',
             'additionalDetails' => [
                'payment_name' => $this->helper->getUpdatedPaymentName($paymentType, $this->helper->getLocaleCodeFromContext($this->context)),
                'change_payment' => true,
                'subscription' => $data
            ]
        ];
        ...
        $this->helper->updateTransactionData($insertData, $this->context);
        ...
        if ($schemaManager->tablesExist(array('novalnet_subscription')) == true) {
            $data = $connection->fetchOne(sprintf("SELECT `id` FROM `novalnet_subscription` WHERE `order_id` = UNHEX('%s')", $this->order->getId()));
            if (!empty($data)) {
                $connection->exec(sprintf("UPDATE `novalnet_subscription` SET `payment_method_id` = UNHEX('%s') WHERE `order_id` = UNHEX('%s')", $input ['custom']['paymentMethodId'], $this->order->getId()));
                $connection->exec(sprintf("UPDATE `novalnet_subs_cycle` SET `payment_method_id` = UNHEX('%s') WHERE `order_id` = UNHEX('%s')", $input ['custom']['paymentMethodId'], $this->order->getId()));
            }
        }
    }
}
