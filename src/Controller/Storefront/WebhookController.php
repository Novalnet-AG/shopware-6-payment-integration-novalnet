...
class WebhookController extends StorefrontController
{
    ...
    public function statusAction(Request $request, SalesChannelContext $salesChannelContext): Response
    {
        ...
        if (!empty($this->eventData ['transaction'] ['order_no'])) {
            ...
            $order = $this->transactionHelper->getOrderEntity($orderNumber, $this->salesChannelContext->getContext());
        ...
        if (! empty($this->eventData ['transaction'] ['order_no']) || ! empty($this->parentTid)) {
            ...
            if (isset($this->eventData ['custom']['change_payment']) && !empty($this->eventData ['custom']['change_payment'])) {
                if ($this->validator->isSuccessStatus($this->eventData)) {
                    ...
                    if (empty($orderData)) {
                        $this->helper->updateChangePayment($this->eventData, $this->order->getId(), $this->salesChannelContext->getContext());
    ...
    public function updateInitialPayment(PaymentMethodEntity $paymentMethod): void
    {
        ...
        if (strpos($handlerIdentifier, 'NovalnetGiropay') !== false || strpos($handlerIdentifier, 'NovalnetSofort') !== false) {
            $comments = $this->helper->formBasicComments($this->eventData, $this->salesChannelContext->getContext());
            $data = [
            ...
            $this->orderTransactionRepository->update([$data], $this->salesChannelContext->getContext());

            if ($this->validator->isSuccessStatus($this->eventData)) {
                // Insert novalnet transaction details
                $insertData = [
                    'id'    => Uuid::randomHex(),
                    'paymentType' => 'novalnet' . strtolower(str_replace('_', '', $this->eventData['transaction']['payment_type'])),
                    'amount' => $this->eventData['transaction']['amount'],
                    'paidAmount' => $this->eventData['transaction']['amount'],
                    ...
                    'gatewayStatus' => $this->eventData['transaction']['status'],
                    'customerNo' => !empty($this->eventData['customer']['customer_no']) ? $this->eventData['customer']['customer_no'] : ''
                ];

                // Upsert data into novalnet_transaction_details.repository
                $this->helper->upsertTransactionData($insertData, $this->salesChannelContext->getContext());
        ...
        $orderTransactionDetails  = $this->transactionHelper->getOrderTransactionDetails($this->orderTransaction->getId(), $this->salesChannelContext->getContext());
        if (method_exists($paymentMethodInstance, 'checkTransactionStatus')) {
            ...
            $paymentMethodInstance->checkTransactionStatus($orderTransactionDetails, $this->eventData, $this->salesChannelContext->getContext(), $paymentTransaction, '1');
            ...
            if ($this->eventType === 'TRANSACTION_CAPTURE') {
                ...
                if (in_array($this->eventData['transaction']['status'], ['CONFIRMED', 'PENDING'])) {
                    if (!empty($this->orderReference->getAdditionalDetails() && !empty($this->orderReference->getPaymentType()) && in_array($this->orderReference->getPaymentType(), ['novalnetinvoice', 'novalnetinvoiceguarantee','novalnetinvoiceinstalment', 'novalnetprepayment']))) {
                        ...
                        $upsertData['additionalDetails'] = $this->eventData['transaction']['bank_details'];
                        if (!empty($upsertData['additionalDetails']['InstalmentDetails'])) {
                            unset($upsertData['additionalDetails']['InstalmentDetails']);
                        }
                    ...
                    if (!empty($upsertData['additionalDetails'])) {
                    ...
                    if (in_array($this->paymentMethodName, ['novalnetinvoice', 'novalnetinvoiceguarantee', 'novalnetinvoiceinstalment'])) {
                        $this->transactionHelper->prepareMailContent($this->order, $this->salesChannelContext->getContext(), $callbackComments);
                    }
                }
    ...
    private function chargebackProcess(): string
    {
        ...
        if (in_array($this->orderReference->getGatewayStatus(), ['CONFIRMED', '100']) && ! empty($this->eventData ['transaction'] ['amount'])) {
            ...
            $this->transactionHelper->postProcess($this->orderTransaction, $this->salesChannelContext->getContext(), $callbackComments);
            $this->helper->updateSubscriptionStatus('CHARGEBACK', $this->orderTransaction->getPaymentMethodId(), $this->order->getId());
        ...
        if (in_array($this->eventData['transaction']['status'], ['PENDING', 'ON_HOLD', 'CONFIRMED', 'DEACTIVATED'])) {
            if (in_array($this->eventData['transaction']['update_type'], ['DUE_DATE', 'AMOUNT_DUE_DATE'])) {
                ...
                if (in_array($this->eventData['transaction']['payment_type'], ['GUARANTEED_INVOICE', 'INSTALMENT_INVOICE', 'INVOICE', 'GUARANTEED_DIRECT_DEBIT_SEPA', 'INSTALMENT_DIRECT_DEBIT_SEPA'])) {
                    $appendComments = false;
                    if (empty($this->eventData['transaction']['bank_details']) && !empty($this->orderReference->getAdditionalDetails())) {
                        $this->eventData['transaction']['bank_details'] = $this->helper->unserializeData($this->orderReference->getAdditionalDetails());
                    }
                    $upsertData['additionalDetails'] = $this->eventData['transaction']['bank_details'];
                    if (!empty($upsertData['additionalDetails']['InstalmentDetails'])) {
    ...
    private function creditProcess(): string
    {
        ...
        if ((int) $this->orderReference->getPaidAmount() < (int) $this->orderReference->getAmount() && in_array($this->eventData['transaction']['payment_type'], [ 'INVOICE_CREDIT', 'CASHPAYMENT_CREDIT', 'MULTIBANCO_CREDIT', 'ONLINE_TRANSFER_CREDIT', 'DEBT_COLLECTION_DE'])) {
            ...
            $amountToBePaid = (int) $this->orderReference->getAmount();
            $callbackComments = $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.creditMessage', [], null, $this->locale), $this->parentTid, $this->formattedAmount, date('d/m/Y H:i:s'), $this->eventTid);
            $this->transactionHelper->postProcess($this->orderTransaction, $this->salesChannelContext->getContext(), $callbackComments, []);
        } else {
        ...
        if (($paidAmount >= $amountToBePaid)) {
            ...
            $this->orderTransactionStateHandler->paidPartially($this->orderTransaction->getId(), $this->salesChannelContext->getContext());
        }
    ...
    private function instalmentProcess(): string
    {
        ...
        $comments .= $this->newLine . $this->newLine . sprintf($this->translator->trans('NovalnetPayment.text.instalmentPrepaidMessage', [], null, $this->locale), $this->parentTid, $this->eventTid, $this->formattedAmount, date('Y-m-d'));
        ...
        $this->transactionHelper->prepareMailContent($this->order, $this->salesChannelContext->getContext(), $comments, true);
        return $comments;
    }
    ...
    private function instalmentCancelProcess(): string
    {
        ...
        if (isset($this->eventData['transaction']['refund'])) {
            ...
            $totalRefundedAmount = $this->orderReference->getRefundedAmount();
    ...
    private function updateInstalmentInfo(): string
    {
        ...
        if (!empty($this->eventData['transaction']['bank_details']['qr_image'])) {
            $configurationDetails['qr_image'] = $this->eventData['transaction']['bank_details']['qr_image'];
        }

        if (!empty($this->eventData ['instalment']['prepaid'])) {
            unset($configurationDetails['qr_image']);
        }
