...
class WebhookController extends StorefrontController
{
    ...
    *   @return Response
    ...
    public function getOrderReference() :bool
    {
        ...
        if (! empty($this->eventData ['transaction'] ['order_no']) || ! empty($this->parentTid)) {
            ...
            $this->locale = (!empty($this->order) && !empty($this->order->getLanguageId())) 
				? $this->helper->getLocaleCodeFromContext($this->salesChannelContext->getContext(), true, $this->order->getLanguageId())
				: $this->helper->getLocaleCodeFromContext($this->salesChannelContext->getContext(), true);
            ...
            if (!empty($this->orderReference)){
				 ...
				 if( ($this->eventType == "PAYMENT" ) && in_array($this->helper->getUpdatedPaymentType($this->orderReference->getPaymentType()), ['CREDITCARD', 'DIRECT_DEBIT_SEPA', 'GOOGLEPAY', 'DIRECT_DEBIT_ACH']) ) {
					 if(isset($this->eventData['transaction']['amount']) && ($this->eventData['transaction']['amount'] > 0) && ($this->orderReference->getAmount() == 0)){ 
							if ($this->helper->isSuccessStatus($this->eventData)) {
								$bookAmountInBiggerUnit = $this->helper->amountInBiggerCurrencyUnit($this->eventData['transaction'] ['amount'], $this->eventData['transaction'] ['currency'], $this->salesChannelContext->getContext());
    ...
    {   
		...
		if(!empty($instalmentData['cycle_amount']) && !empty($instalmentData['cycles_executed']) && !empty($instalmentData['pending_cycles'])){
