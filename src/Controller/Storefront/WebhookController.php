    ...
    *   @return Response
    ...
    public function getOrderReference() :bool
    {
        ...
        if (! empty($this->eventData ['transaction'] ['order_no']) || ! empty($this->parentTid)) {
            ...
            if (!empty($this->orderReference) && ($this->eventData['transaction']['amount'] > 0) &&
                ($this->orderReference->getAmount() == 0 &&
                in_array($this->helper->getUpdatedPaymentType($this->orderReference->getPaymentType()), ['CREDITCARD', 'DIRECT_DEBIT_SEPA', 'GOOGLEPAY']) )) {
                if ($this->helper->isSuccessStatus($this->eventData)) {
