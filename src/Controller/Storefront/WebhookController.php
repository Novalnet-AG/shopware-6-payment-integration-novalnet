...
class WebhookController extends StorefrontController
{
    ...
    private function transactionCaptureVoid(): string
    {
        ...
        if (in_array($this->orderReference->getGatewayStatus(), ['ON_HOLD', 'PENDING']) || in_array($this->orderReference->getGatewayStatus(), ['98', '99', '91', '85', '90', '86', '75'])) {
            ...
            if (in_array($this->paymentMethodName, ['novalnetinvoice', 'novalnetinvoiceguarantee', 'novalnetinvoiceinstalment', 'novalnetsepainstalment', 'novalnetsepaguarantee']) && in_array($this->eventData['transaction']['status'], ['CONFIRMED', 'PENDING'])) {
    ...
    private function transactionUpdate(): string
    {
        ...
        if (in_array($this->paymentMethodName, ['novalnetinvoice', 'novalnetinvoiceguarantee', 'novalnetinvoiceinstalment', 'novalnetsepainstalment', 'novalnetsepaguarantee']) && in_array($this->eventData['transaction']['status'], ['CONFIRMED', 'PENDING', 'ON_HOLD'])) {
            $this->transactionHelper->prepareMailContent($this->order, $this->salesChannelContext, $callbackComments);
        }
