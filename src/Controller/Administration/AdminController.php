...
class AdminController extends AbstractController
{
     ...
     */
    public function __construct(private readonly NovalnetHelper $helper, private readonly TranslatorInterface $translator, private readonly NovalnetOrderTransactionHelper $transactionHelper)
    {
    }
    ...
    public function loadNovalnetPaymentForm(Request $request, Context $context): JsonResponse
    {
        ...
        if (!empty($request->get('shippingaddress')) && !empty($request->get('billingaddress')) && !empty($request->get('amount')) && !empty($request->get('currency')) && !empty($request->get('customer'))) {
            ...
            $customer = $this->helper->getCustomerDetails($request->get('customer')['id'], $context);
            $response = $this->helper->getNovalnetIframeResponse($request->get('customer')['salesChannel']['id'], $customer, $requiredFields, $context, 'BACKEND');
        }
    ...
    #[Route(path: '/api/_action/novalnet-payment/customer-payment-data', name: 'api.action.novalnet.payment.customer.payment.data', methods: ['POST'])]
    public function getPaymentDetails(Request $request, Context $context): JsonResponse
    {
        $response = [];
        if (!empty($request->get('customerNo'))) {
            $result = $this->transactionHelper->getCustomerPaymentDetails($context, $request->get('customerNo'));
            $response = $result;
        }
