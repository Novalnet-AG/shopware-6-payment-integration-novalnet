    ...
    public function managePaymentTransaction(Request $request, Context $context): JsonResponse
    {
        ...
        if ($request->get('status') && $request->get('orderNumber')) {
            ...
            if (!is_null($transactionData)) {
                ...
                if($status == 'transaction_capture' && $transactionData->getPaymentType() =='novalnetpaypal' && $response['transaction']['status'] == 'CONFIRMED')
                {
					$data = [
							'tid'	=> $transactionData->getTid(),
							'accountData' => $response['transaction']['payment_data']['paypal_account']
					];
					$existingToken = $this->getExistingPaymentToken($context, $data);
					if(!is_null($existingToken))
					{
						$data['id'] = $existingToken->getId();
