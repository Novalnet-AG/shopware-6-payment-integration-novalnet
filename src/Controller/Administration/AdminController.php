...
class AdminController extends AbstractController
{
    ...
    */
        ...
        if (!empty($request->get('orderNumber'))) {
            ...
            if (!empty($transactionData)) {
                ...
                if (isset($paymentmethod['payment_name']) && ! empty($paymentmethod['payment_name'])) {
                    $paymentMethodsName = $paymentmethod['payment_name'];
                }
            }
            
            $paymentMethodName = ['paymentName' => $paymentMethodsName];
        }
        
        return new JsonResponse($paymentMethodName);
    }
    ...
    /**
     * @Route(
     *     "/api/v{version}/_action/novalnet-payment/shop-version",
     *     name="api.action.noval.payment.compatibility.shop.version",
     *     methods={"GET","POST"}
     * )
     *
     * @Route(
     *     "/api/_action/novalnet-payment/shop-version",
     *     name="api.action.noval.payment.shop.version",
     *     methods={"GET","POST"}
     * )
     */
    public function getAdminShopVersion(): JsonResponse
    {
        $result = $this->helper->getShopVersion();
        return new JsonResponse(['version' => $result]);
    }
}
