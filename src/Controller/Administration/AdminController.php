...
class AdminController extends AbstractController
{
    ...
    public function validateApiCredentials(Request $request, Context $context): JsonResponse
    {
        ...
        if ($request->get('clientId') && $request->get('accessKey')) {
            ...
            $response = $this->helper->sendPostRequest(
                $parameters,
                $this->helper->getActionEndpoint('merchant_details'),
    ...
    public function refundAmount(Request $request, Context $context): JsonResponse
    {
        ...
        if ($request->get('orderNumber')) {
            ...
            $transactionData = $this->transactionHelper->fetchNovalnetTransactionData(
            ...
            if (!is_null($transactionData)) {
                ...
                if ((int) $transactionData->getAmount() <= (int) $transactionData->getRefundedAmount()) {
                    return new JsonResponse(['result' => ['status_text' =>
                    $this->translator->trans('NovalnetPayment.text.refundAlreadyExists')]]);
                }
                if ((int) $transactionData->getAmount() < $refundedAmount) {
                    return new JsonResponse(['result' => ['status_text' =>
                    $this->translator->trans('NovalnetPayment.text.invalidRefundAmount')]]);
                }

                if (!is_null($orderReference)) {
                    $response =  $this->transactionHelper->refundTransaction(
                        $transactionData,
                        $orderReference,
                        $context,
                        (int) $refundedAmount,
    ...
    public function instalmentCancel(Request $request, Context $context): JsonResponse
    {
        ...
        if ($request->get('orderNumber')) {
            ...
            if (!is_null($transactionData)) {
                $orderReference  = $this->transactionHelper->getOrder(
                ...
                if (!is_null($orderReference)) {
                    $response =  $this->transactionHelper->cancelInstalmentPayment(
                        $transactionData,
                        $orderReference,
                        $context,
                        $request
    ...
    public function bookAmount(Request $request, Context $context): JsonResponse
    {
        ...
        if ($request->get('orderNumber') && $bookedAmount) {
            ...
            if (!is_null($transactionData)) {
                $orderEntity = $this->transactionHelper->getOrderEntity(
                    $request->get('orderNumber'),
                    $context
                );
                ...
                if (!is_null($orderEntity)) {
                    $response =  $this->transactionHelper->bookOrderAmount(
                        $bookedAmount,
                        $transactionData,
                        $orderEntity,
                        $context
        ...
        if ($request->get('status') && $request->get('orderNumber')) {
            ...
            if (!is_null($transactionData)) {
                ...
                if (!is_null($orderReference)) {
                    ...
                    $response =  $this->transactionHelper->manageTransaction(
                        $transactionData,
                        $orderReference,
                        $context,
                        $status
    ...
    public function configureWebhook(Request $request, Context $context): JsonResponse
    {
        if (!empty($request->get('url')) &&
            !empty($request->get('productActivationKey'))
            && !empty($request->get('paymentAccessKey'))) {
            $parameters = [
                    'merchant' => [
            ...
            $response = $this->helper->sendPostRequest(
                $parameters,
                $this->helper->getActionEndpoint('webhook_configure'),
                $request->get('paymentAccessKey')
            );
            return new JsonResponse($response);
        }
        return new JsonResponse(['result' => '']);
